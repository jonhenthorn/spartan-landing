...
## Current status: live schema 3 remains inert; schema-4 Apps health candidate is local only

The bounded D1 monitoring evaluator is implemented, locally validated and remotely proven in the isolated sandbox. The counts-only alert planner/drainer and migration `0002` are deployed inertly, but no email binding, destination or sender is configured. The least-privilege Queue/DLQ metrics source and migration `0003` are applied and deployed inertly with the source flag false and no Queue-read token. A separately gated schema-4 candidate now adds a signed, read-only Apps Script health source and migration `0004`; it is locally implemented only and has not been migrated, configured, deployed or called in sandbox or production. The permanent sandbox service remains **inert and not production-activation-ready**:

- The Worker exports only a scheduled handler. It has no `fetch` handler, public route, custom domain or `workers.dev` exposure.
- Production and sandbox are separate, non-inheriting Wrangler files. Every currently deployed capability flag and every checked-in schema-4 candidate flag is `false`.
...
- The monitor can run only on the exact five-minute cron when `OPS_MONITORING_ENABLED=true`. It makes four fixed aggregate-only `SELECT` queries against connector D1 and writes only non-PII run/incident evidence to operations D1. The schema-4 candidate independently gates Queue metrics with `OPS_QUEUE_MONITORING_ENABLED` and Apps health with `OPS_APPS_SCRIPT_MONITORING_ENABLED`; both require the aggregate monitor.
...
Migrations `0001_ops_state.sql` through `0004_apps_script_health_alerts.sql` define only non-PII operational metadata. Migrations `0001`–`0003` are applied to both sandbox databases; candidate `0004` is not. Migration `0004` clones the schema-3 27-column delivery table exactly, preserves all prior rows, indexes and condition/reason pairs with atomic guards, and adds only the three fixed Apps health pairs. No production database exists:
...
5. Deploy the matching reviewed Apps Script health contract first, apply migration `0004` preview then runtime with preservation proof, install a new dedicated sandbox health secret and URL outside Git, and prove signed healthy/disabled/failure/configuration/recovery behavior before returning `OPS_APPS_SCRIPT_MONITORING_ENABLED=false` and removing the test values if they will not remain managed. The local candidate and expected-state placeholders are not activation approval.
