...
## Current status: live schema 4 is inert after a failed Apps-health deadline gate

The bounded D1 monitoring evaluator is implemented, locally validated and remotely proven in the isolated sandbox. The counts-only alert planner/drainer and migration `0002` are deployed inertly, but no email binding, destination or sender is configured. The least-privilege Queue/DLQ metrics source and migration `0003` are applied and deployed inertly with the source flag false and no Queue-read token. Migration `0004`, the schema-4 signed Apps Script health source and the matching Apps Script v3 contract are also deployed inertly. A bounded live sandbox run passed signed-disabled checks but stopped when the first enabled inspection took `5016 ms`, 16 ms beyond the strict deadline. The immediate repeat was healthy in `3878 ms`, but it did not override the stop rule. The Apps endpoint is again disabled, the operations Worker has no health URL or shared secret, and another live retry is not approved. The permanent sandbox service remains **inert and not production-activation-ready**:
...
- Final sandbox Worker version `65e97390-997e-46e8-9afa-f8721c644ef0` has only the scheduled handler, is bound to runtime operations D1 `2e2fc9f6-0a81-453b-9af6-8d4104965f8e` and real sandbox connector D1 `9531221e-cabe-4ed4-b7d4-f715798b8945`, has schema `4`, no route, `workers.dev` hostname, secret, Queue, email or R2 binding, and retains every capability flag `false`. The post-cleanup cron changed no count or prior update timestamp. Runtime now preserves 17 monitor runs, three incidents and one open `APPS_HEALTH_UNAVAILABLE` warning from the failed timing gate; deliveries, backups and restores remain zero.
