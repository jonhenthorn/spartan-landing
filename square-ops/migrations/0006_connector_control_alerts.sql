...
CREATE TABLE alert_delivery_v6_migration_guard (
  invalid_evidence_count INTEGER NOT NULL CHECK (invalid_evidence_count = 0)
);

INSERT INTO alert_delivery_v6_migration_guard (invalid_evidence_count)
SELECT COUNT(*)
 ...
 WHERE (delivery.channel_code = 'OWNER_EMAIL' AND delivery.target_role_code <> 'OWNER')
    ...
    OR NOT (
      ...
      (delivery.alert_key = 'DELIVERY_RETRY_ATTEMPT_3' AND delivery.reason_code = 'DELIVERY_RETRY_ATTEMPT_THRESHOLD_REACHED') OR
      (delivery.alert_key = 'STALE_LEASE_SURVIVED_RECOVERY' AND delivery.reason_code = 'PROCESSING_LEASE_SURVIVED_RECOVERY_CYCLE') OR
      (delivery.alert_key = 'STAFF_LOOKUP_THRESHOLD' AND delivery.reason_code = 'STAFF_LOOKUP_RATE_OR_COUNT_ELEVATED') OR
      (delivery.alert_key = 'DISCOUNT_POLICY_DISABLE_REQUIRED' AND delivery.reason_code = 'OFFER_PASS_DISABLE_REQUIRED') OR
      ...
      (delivery.alert_key = 'PROVIDER_OUTCOME_UNAVAILABLE' AND delivery.reason_code = 'PROVIDER_OUTCOME_SOURCE_UNAVAILABLE') OR
      (delivery.alert_key = 'SQUARE_PROVIDER_AUTH_REJECTED' AND delivery.reason_code = 'SQUARE_PROVIDER_AUTH_OR_SCOPE_REJECTED') OR
      (delivery.alert_key = 'PROVIDER_RATE_WARNING' AND delivery.reason_code = 'PROVIDER_RATE_OR_SERVER_FAILURE') OR
      (delivery.alert_key = 'ALERT_PATH_TEST' AND delivery.reason_code = 'MONTHLY_ALERT_PATH_TEST')
    );

CREATE TABLE alert_deliveries_v6 (
  ...
  CHECK (
    ...
    (alert_key = 'DELIVERY_RETRY_ATTEMPT_3' AND reason_code = 'DELIVERY_RETRY_ATTEMPT_THRESHOLD_REACHED') OR
    (alert_key = 'STALE_LEASE_SURVIVED_RECOVERY' AND reason_code = 'PROCESSING_LEASE_SURVIVED_RECOVERY_CYCLE') OR
    (alert_key = 'STAFF_LOOKUP_THRESHOLD' AND reason_code = 'STAFF_LOOKUP_RATE_OR_COUNT_ELEVATED') OR
    (alert_key = 'DISCOUNT_POLICY_DISABLE_REQUIRED' AND reason_code = 'OFFER_PASS_DISABLE_REQUIRED') OR
    ...
    (alert_key = 'PROVIDER_OUTCOME_UNAVAILABLE' AND reason_code = 'PROVIDER_OUTCOME_SOURCE_UNAVAILABLE') OR
    (alert_key = 'SQUARE_PROVIDER_AUTH_REJECTED' AND reason_code = 'SQUARE_PROVIDER_AUTH_OR_SCOPE_REJECTED') OR
    (alert_key = 'PROVIDER_RATE_WARNING' AND reason_code = 'PROVIDER_RATE_OR_SERVER_FAILURE') OR
...
INSERT INTO alert_deliveries_v6 (
...
SELECT ABS(
  ...
  (SELECT COUNT(*) FROM alert_deliveries_v6)
);

DROP TABLE alert_delivery_v6_migration_guard;

DROP TABLE alert_deliveries;
ALTER TABLE alert_deliveries_v6 RENAME TO alert_deliveries;
