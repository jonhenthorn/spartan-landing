...
Do not enable any feature flag, add a canary submission or create a customer/group write until the remaining sandbox gates pass.
...
Rollback is flag-only: disable `SQUARE_OFFER_ENABLED` first so the established coupon remains usable, then disable webhook/consumer/pass/reconciliation. Preserve D1, Queue, DLQ, Square customers, and Apps ledger evidence for review; do not delete or silently rewrite history.
