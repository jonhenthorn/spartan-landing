...
The normal sandbox boundary is default-off. For the bounded reconciliation proof, only consumer and reconciliation were temporarily enabled; offer, webhook, pass and owner-harness remained `false`, canary-only retained an empty allowlist, the four-event Square webhook subscription remained disabled and sandbox Apps `SQUARE_JOURNEY_ENABLED=false`. Reconciliation was turned off first after the second cycle, terminal/dead state was rechecked, and then the consumer was turned off. Worker version `6081f8ec-3626-4731-aa3d-c1abf065e2d9` was the historical all-off result of that proof. Worker version `0ff5a2ab-2f2c-4872-a624-29d976ab54de`, created by the August 19 sandbox credential-remediation rotation and bound to runtime D1 `9531221e-cabe-4ed4-b7d4-f715798b8945`, was the next historical all-off baseline. During the closed August 21 baseline migration, one separately reviewed all-off target became the sole sandbox baseline; its exact current version and binding references remain in private evidence rather than this shared handoff. All five automation flags and owner-harness remain `false`, canary-only is `true` with an empty allowlist, and the public configuration reports `enabled:false`. Both historical versions were retained. There is no deployed production connector route, credential or automation, and the existing production website, form Worker, Square account and Brevo flow were unchanged.
...
The bounded execution plan for those connector negative/recovery cases is `docs/SQUARE-SANDBOX-NEGATIVE-RECOVERY-ACCEPTANCE.md`. It distinguishes local proof from live provider acceptance. Default-inert credential-safe drivers now cover the filtered Apps request and webhook integrity/replay request construction, and a separate local helper verifies exact named Queue boundaries plus one-visible-message DLQ inspect/redrive behavior. The first F-02 attempt safe-stopped during its initial aggregate read-only D1 capture with HTTP `000` and requests `0`, before candidate traffic or any request; no case has been accepted. That expired authority cannot be reused. The sandbox fault hook and DLQ helper still require a fresh approved sandbox deployment/credential/Queue window; the worksheet is not approval to deploy, retry or run a case.

The exact-target hook boundary and masked-input preparation procedure are documented in `docs/SQUARE-SANDBOX-FAULT-HOOKS.md`. Production still bundles `src/index.mjs`; only the sandbox config bundles `src/sandbox.mjs`, and its fault flag defaults false. A previously reviewed controller build is deployed only as the current all-off sandbox baseline. No case profile, candidate, canary or temporary control is active or armed, and the newer F-02/P-02 observer repairs are reviewed offline code only and have not been deployed.
...
Cloudflare's production-named `spartan-square-connector-dlq` must be inspected by an authorized owner, but this repository does not claim a generic Wrangler replay command. The implemented helper and procedure in `docs/SQUARE-DLQ-REDRIVE.md` are sandbox-only, exact-target and require separate inspect/redrive approval. Any future production recovery needs its own reviewed tool and authorization. Never retry an unconfirmed push blindly, delete DLQ messages, reset D1 attempts or manually change ledger state merely to silence an alert. The connector's refund path remains review-only during recovery and never automatically reissues the offer.
...
Rollback is control-first and non-deleting: disable offer, pass and the sandbox owner harness first, then empty the canary allowlist so the established coupon remains usable and no new Square offer appears. Keep verified webhook/consumer processing enabled only long enough to drain accepted work; then disable the Square subscription, webhook, consumer and reconciliation flags, and disable Apps journey processing last. Rotate a credential only if exposure requires it. Preserve D1, Queue, DLQ, Square customers and Apps ledger evidence for review; do not delete or silently rewrite history.
