...
- `square-ops/` — the scheduled-only, default-off operations plane. Its aggregate D1 monitor, unbound counts-only alert engine and read-only Queue/DLQ source remain deployed inertly on schema 3 in the isolated sandbox. A schema-4 signed Apps Script health source is a local candidate only: migration `0004`, the dedicated URL/secret and the new Worker are not deployed or activated. Every live capability is false, and the service has no public route.
...
The candidate uses a separate Cloudflare Worker, verified webhooks, D1 idempotency state, a Queue and dead-letter queue, default-off controls and a one-submission canary. The isolated D1 monitor is remotely proven; alert migration `0002`, Queue-source migration `0003` and their unbound counts-only engines are deployed inertly in sandbox. The Queue source remains off with no token or message-operation binding. A signed, read-only Apps health source and schema-4 alert-pair migration are locally validated candidates only; no health URL/secret, migration, request or Worker deployment has occurred. Deploy-only email destinations, live external delivery, production credentials, remaining monitoring sources, recurring backups, retention and the production canary remain mandatory gates in [`docs/SQUARE-CONNECTOR-ROLLOUT.md`](docs/SQUARE-CONNECTOR-ROLLOUT.md). Do not add Square credentials or webhook processing to the current form proxy.
