...
- `scripts/manage-square-sandbox-oauth.mjs` and `scripts/validate-square-sandbox-oauth.mjs` — a default-inert, local-contract-only lifecycle for separate read-only and mutation Sandbox OAuth apps, exact case scopes, loopback callback validation, one-attempt/fail-sticky admission, full authorization revocation, access/refresh retirement proof and standing-connector drift checks. The public path remains compiled shut and returns `CREDENTIAL_GATE_BLOCKED`; it has no registered app IDs or secret digests, configured shared admission/custody backing, trusted clock, callback listener, Square transport or case adapter, and it is not integrated with the provider-fixture helper. Local `.invalid` tests make no network request and grant no live authority.
...
- `square-ops/` — the scheduled-only, default-off operations plane. Its aggregate D1 monitor, unbound counts-only alert engine, read-only Queue/DLQ source and signed Apps Script health source are deployed inertly on schema 4 in the isolated sandbox. The Option B Apps-health sandbox lane passed its bounded acceptance worksheet; the final all-off operations Worker version `12bd4dc9-3ed7-47e0-9c48-0c33d8a5c166` has no operations secrets, all six live capability flags are false, and the service has no public route. Preserved evidence is 34 monitor runs, five resolved incidents, zero active incidents, and zero deliveries, backups or restores. A newer local schema-5 candidate adds default-off, fixed-class provider monitoring: one Square `401/403` is latched critical, three combined Square/Apps `429/5xx` outcomes in 15 minutes warn, and empty results resolve only with a fresh producer heartbeat and no unresolved provider-attempt proof. Any admitted attempt that cannot close atomically remains `PENDING` or `FAULTED`, makes the source unavailable and blocks incident resolution; 30-day retention never deletes that proof. The candidate is not deployed or remotely accepted.
