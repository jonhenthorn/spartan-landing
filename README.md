...
- `docs/PROJECT-2-OPERATIONS-ACTIVATION-DECISION-RECORD.md` — the blank default-NO-GO template for one separately selected monitoring, alert, backup, restore or operational-control lane; it separates implementation, inert deployment, live acceptance and continuing operation.
- `docs/PROJECT-2-PRODUCTION-ACTIVATION-DECISION-RECORD.md` — the blank default-NO-GO template for one production owner-canary attempt; it separates resource preparation, inert deployment, canary activity, rollback and later customer use.
- `scripts/project2-production-canary-contract.mjs` and `scripts/validate-project2-production-canary-contract.mjs` — a public-compiled-shut, synthetic-only local contract for exact production-canary admission, separate final owner GO, one-canary stage ordering, immediate rollback, interruption fencing, retained history and all-off closure. Fixed internal scenarios prove replay, drift, split-traffic, late-effect and incomplete-cleanup refusal without a credential, provider adapter, network request, deployment or production identifier; it does not authorize or perform a canary.
...
- `docs/SQUARE-SANDBOX-PROVIDER-FIXTURES.md` — the validation-only provider boundary for five mutating fixtures plus credential-blocked, zero-mutation F-04, P-01 and package-bound replay preflights. F-04/P-01 use `--execute-read-only --case <F-04|P-01> --ack SANDBOX_PROVIDER_READ_ONLY_PREFLIGHT_ONLY`; replay uses `--execute-read-only --case REPLAY-4XX --package "<secured-replay-package-path>" --ack SANDBOX_PROVIDER_READ_ONLY_PREFLIGHT_ONLY`. Local validation is restricted to `https://provider-fixture.invalid`, while the fixture helper's approved clients remain compiled `null`. No preflight or fixture has run live.
- `scripts/manage-square-sandbox-oauth.mjs`, `scripts/square-sandbox-oauth-fixture-bridge.mjs` and their focused validators — a default-inert, local-contract-only lifecycle for separate read-only and mutation Sandbox OAuth apps plus a validation-only opaque-handle bridge into the bounded provider-fixture helper. Local `.invalid` tests cover exact case scopes, callback validation, one-attempt/fail-sticky admission, full authorization revocation, access/refresh retirement, standing-connector drift and the composed fixture boundary without making a network request. The public path remains compiled shut and returns `CREDENTIAL_GATE_BLOCKED`: no reviewed dedicated app registration or live configuration exists, live client IDs and secret digests remain absent, and there is no OS-durable shared admission/custody adapter, trusted live clock, callback listener, provider transport or live authorization. The local bridge grants no provider or production authority.
- `square-ops/src/backup-restore-contract.mjs` and `scripts/validate-square-ops-backup-restore-contract.mjs` — a default-inert, local-contract-only boundary for a schema-complete D1 export, exact private-object identity, isolated restore integrity and foreign-key checks, deletion-manifest replay, exact-key deletion proof and failure-safe cleanup. Its public path remains compiled shut and returns `LOCAL_CONTRACT_ONLY_LIVE_NOT_READY`: no R2 bucket/binding, scheduled writer, storage identity adapter, deletion-manifest store, restore executor or live evidence is configured. Local synthetic validation creates no Cloudflare resource and grants no backup, restore, deletion or production authority.
- `square-ops/` — the scheduled-only, default-off operations plane. Its aggregate D1 monitor, unbound counts-only alert engine, read-only Queue/DLQ source and signed Apps Script health source are deployed inertly on schema 4 in the isolated sandbox. The Option B Apps-health sandbox lane passed its bounded acceptance worksheet; the final all-off operations Worker version `12bd4dc9-3ed7-47e0-9c48-0c33d8a5c166` has no operations secrets, all six live capability flags are false, and the service has no public route. Preserved evidence is 34 monitor runs, five resolved incidents, zero active incidents, and zero deliveries, backups or restores. A newer local schema-6 candidate retains the default-off provider-outcome contract and adds four aggregate connector signals: retry attempt 3, a processing lease surviving its five-minute recovery cycle, an elevated daily staff-lookup count/rate and repeated discount-policy rejection requiring offer/pass disablement. It also adds a separately gated, at-most-once-per-month `TEST` delivery for each owner role. The checked-in schema-6 flags all remain false; migrations `0005` and `0006`, the new signals and the monthly alert-path test have not been deployed, migrated, sent or remotely accepted.
...
Only the sandbox Apps-health lane is complete. The provider helper now has a locally validated read-only preflight contract for a point-in-time F-04 or P-01 new-customer-slot check and one exact replay-package target. A validation-only OAuth bridge composes that helper with synthetic issuance and retirement under `.invalid` transport, but every public client/live bridge gate remains closed and no provider call has run. A future successful preflight would still prove neither Apps readiness nor Worker, Queue, D1 or provider mutation behavior; replay success would show only an authorized permanent rejection for that exact package target, never object absence. The Queue source remains off with no token or message-operation binding. Deploy-only email destinations, live external delivery, Queue acceptance, backup and restore implementation, production credentials, remaining monitoring sources, retention and the production canary remain mandatory gates in [`docs/SQUARE-CONNECTOR-ROLLOUT.md`](docs/SQUARE-CONNECTOR-ROLLOUT.md). Do not add Square credentials or webhook processing to the current form proxy.
