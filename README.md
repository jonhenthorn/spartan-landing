...
- `square-ops/` — the scheduled-only, default-off operations plane; its aggregate D1 monitor, unbound counts-only alert engine and read-only Queue/DLQ source are deployed inertly on schema 3 in the isolated sandbox. Every capability is false and the Queue source has no token. Backups/restores remain fail-closed, and the service has no public route.
