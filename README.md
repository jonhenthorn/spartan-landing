...
- `docs/APPS-HEALTH-SANDBOX-ACCEPTANCE.md` — the owner-controlled signed Apps-health test matrix, direct-verifier contract, stop rules and exposure-first rollback worksheet. Its August 18 run stopped at the first enabled-inspection deadline and returned fully to the default-off state; another live retry is not approved.
...
The candidate uses a separate Cloudflare Worker, verified webhooks, D1 idempotency state, a Queue and dead-letter queue, default-off controls and a one-submission canary. The isolated D1 monitor is remotely proven; alert migration `0002`, Queue-source migration `0003`, Apps-health migration `0004` and their unbound counts-only engines are deployed inertly in sandbox. The matching Apps Script v3 contract is published with health disabled. A bounded signed Apps-health run passed disabled checks but stopped when its first enabled inspection took `5016 ms`, 16 ms beyond the strict deadline; the immediate repeat was healthy in `3878 ms`. Cleanup removed the Worker-held Apps-health URL/secret entries and Apps health property, returned every operations flag to false and preserved the open fixed-code warning with zero deliveries. The Queue source remains off with no token or message-operation binding. Deploy-only email destinations, live external delivery, source credentials and live tests, production credentials, remaining monitoring sources, recurring backups, retention and the production canary remain mandatory gates in [`docs/SQUARE-CONNECTOR-ROLLOUT.md`](docs/SQUARE-CONNECTOR-ROLLOUT.md). Do not add Square credentials or webhook processing to the current form proxy.
