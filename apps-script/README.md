...
- Apps-health acceptance remains incomplete. Option B is approved and locally validated in the operations candidate: one shared `10000 ms` transport deadline, no retry and strict raw `<8000 ms` acceptance, with integrity failure taking precedence over timing. The candidate is not yet deployed or live-proven. The complete stopped-run evidence, revised policy and cleanup contract are recorded in `docs/APPS-HEALTH-SANDBOX-ACCEPTANCE.md`.
