...
- The v1 health contract marker is present in the saved code, while the public GET still returns the existing v3.2 form-service contract. An approved signed sandbox run returned two valid `DISABLED` responses in `1500 ms` and `1485 ms`. After health was enabled, the first metadata inspection exceeded the hard deadline at `5016 ms`; an immediate repeat returned valid signed `COMPLETE`, configuration healthy, in `3878 ms`.
- The first enabled result triggered the worksheet stop rule. The forced-environment and configuration-mismatch phases were not run. Emergency cleanup restored health false/environment sandbox, removed the dedicated Apps property and both Worker secrets, returned all operations flags to false, and produced a final all-off no-write cron. No Sheet value, customer data, mail, Brevo, Square or production state was changed.
- Another live retry is blocked. First optimize the metadata inspection or approve a separately reviewed deadline change, update the tests and worksheet, then obtain a new explicit approval. The failed-run evidence and exact cleanup are recorded in `docs/APPS-HEALTH-SANDBOX-ACCEPTANCE.md`.
- A local, undeployed exact-semantic optimization now reuses the write-secret values already read for credential-separation checks, skips owner/Square properties while those optional lanes are disabled, and enumerates workbook tabs once for the lead and both ledger checks. It retains the complete used-range formula scan and every allocated-row identifier-format check. The validator locks the exact property/read plan, one tab enumeration, zero `getSheetByName()` calls, no data-row value reads and future unused-row format drift remaining `NOT_READY`. Live Apps Version `3`, its disabled properties and the five-second gate are unchanged.
