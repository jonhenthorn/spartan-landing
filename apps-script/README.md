...
- The existing sandbox web-app deployment now serves optimized Version `4` without changing its deployment, execute-as owner or `Anyone` access. Health remained disabled and the next all-off operations interval wrote nothing.
...
- The v1 health contract marker remains present, while the public GET still returns the existing v3.2 form-service contract. The August 18 run returned two valid `DISABLED` responses, then stopped at a `5016 ms` enabled-inspection deadline. The immediate `3878 ms` healthy repeat did not override that stop.
- The August 19 follow-up passed a credential-inert interval and two signed `DISABLED` responses in `1791 ms` and `1009 ms`. It stopped before health enablement when automation output disclosed private Apps configuration. The exposed sandbox connector signing secret was replaced in Apps and the disabled connector. Healthy, forced-environment, mismatch and recovery phases were not run.
- Cleanup removed the dedicated health property and both Worker health secrets, returned every operations flag to false and produced a final all-off no-write cron. A final exact-selector attempt stopped before any credential was saved or sent; its unsaved row and unused temporary credential were removed. No Sheet value, customer data, mail, Brevo, Square business record or production state changed.
...
- A later enabled Worker run at `06:30` ended `APPS_HEALTH_SECOND_HOP_UNAVAILABLE` in `2966 ms`, while the Apps execution UI showed Version 4 `doPost` completed in `2.069 s`. That difference did not prove a signed response reached the Worker, so the worksheet hard stop and cleanup ran immediately.
...
- A fresh credential-local attempt produced signed `DISABLED` diagnostic evidence in `5422 ms` and `1585 ms`. Diagnostic mode cannot satisfy acceptance: the `5422 ms` result is outside the current strict `<5000 ms` SLO. The first strict normal probe then failed at `5011 ms`, and the run stopped before any Worker health secret was added or any capability flag was enabled.
- Cleanup removed the dedicated credential and private URL entries. Apps health is disabled, the dedicated secret is absent, the connector and production were unchanged, and no Sheet value, customer data, provider action or business record changed. Final operations evidence is 22 monitor runs, three incidents, one preserved open Apps warning at occurrence one, and zero deliveries, backups or restores.
- Apps-health acceptance remains incomplete. Before another attempt, an explicit policy decision must either keep the five-second gate and replace or abandon the synchronous Apps source, or separately approve a 10-second transport ceiling with strict `<8000 ms` acceptance. The complete stopped-run evidence and cleanup are recorded in `docs/APPS-HEALTH-SANDBOX-ACCEPTANCE.md`.
