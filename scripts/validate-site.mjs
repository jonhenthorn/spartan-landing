...
assert.match(siteScript, /const showUpdatesConfirmation = \(\) =>/);
assert.match(siteScript, /updatesForm\?\.setAttribute\("hidden", ""\)/);
assert.match(siteScript, /updatesConfirmation\?\.removeAttribute\("hidden"\)/);
assert.match(siteScript, /updatesGrid\?\.setAttribute\("data-updates-confirmed", "true"\)/);
assert.match(siteScript, /updatesConfirmation\?\.scrollIntoView\(\{ block: "start", behavior: "instant" \}\)/);
assert.match(siteScript, /updatesConfirmationTitle\?\.focus\(\{ preventScroll: true \}\)/);
assert.match(siteScript, /window\.addEventListener\("load", focusConfirmation, \{ once: true \}\)/);
assert.match(homepage, /data-updates-confirmation[\s\S]*role="region"[\s\S]*aria-labelledby="updates-confirmation-title"[\s\S]*hidden/);
assert.match(homepage, /id="updates-confirmation-title" tabindex="-1" data-updates-confirmation-title/);
assert.match(homepage, /Your Spartan Updates subscription is confirmed\./);
assert.match(homepage, /data-track="menu_click" data-track-location="updates_confirmation"/);
assert.match(homepage, /data-track="directions_click"[\s\S]*data-track-location="updates_confirmation"/);
assert.match(siteStyles, /\.updates-confirmation-card/);
assert.match(siteStyles, /\.updates-confirmation-card\[hidden\]\s*\{\s*display: none;/);
assert.match(siteStyles, /\.updates-grid\[data-updates-confirmed="true"\] \.updates-confirmation-card\s*\{\s*order: -1;/);
