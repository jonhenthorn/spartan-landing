...
Status: **implemented and locally testable. A previously reviewed default-off controller build is deployed only as the current all-off sandbox baseline; the newer F-02 hardening in this draft is not deployed. No F-02 or other live-case candidate, profile, canary or temporary control is active, armed or approved.**
...
Handled `SIGINT` or `SIGTERM` emits exactly one fixed terminal stop: `STATUS=STOPPED RESULT=STOP_F02_DRIVER_INTERRUPTED HTTP=000 REQUESTS=0` only before the fetch-attempt marker, or the same result with `REQUESTS=1` from immediately before the only fetch invocation onward. The count never returns to zero and the interrupted process never retries or invokes rollback. Treat either fixed interrupt result—and any unhandled termination or exit without a fixed terminal line—as stop/ambiguity: do not rerun the request, and use the separately authorized exact rollback action immediately if the candidate may be active.
