...
Status: **implemented and locally testable. A previously reviewed default-off controller build is deployed only as the current all-off sandbox baseline; the newer F-02 and P-02 observer repairs in the reviewed repository source are not deployed. The first F-02 attempt safe-stopped during its initial aggregate read-only D1 capture with `F02_ZERO_REQUEST_SAFE_STOP_CLOSURE_CONFIRMED`, HTTP `000` and requests `0`, before candidate traffic or any request. Three later August 23 preparation/readiness attempts also closed before any candidate version, traffic assignment or request, and their temporary credentials were revoked and proved unusable. An August 25 preparation/readiness attempt stopped at strict window-end staging because a seconds-only UTC value was not in the then-required millisecond form. It reached no helper, operator, candidate, coordinator, traffic or request boundary; the all-off baseline remained the sole 100% deployment, both Queues remained reported empty, and the temporary `W` and `R` credentials were revoked and proved unusable by the required one-plus-three HTTP `401` checks. No live-case candidate, profile, canary or temporary control is currently active or armed, and no unexpired F-02 or other case authority exists. The expired F-02 authority cannot be reused.**
...
The namespace start must be current within five minutes. After validating the exact command and initial freshness, the command prints `ACTION=TYPE_START_ACKNOWLEDGEMENT_MANUALLY_DO_NOT_PASTE`, then prompts for `START_F02_COPY_TEST_ONCE`; this first acknowledgement occurs before any pasteboard access. Type both preflight acknowledgements manually and never paste them. Their dedicated visible nonsecret reader renders only the known canonical next character after an exact match, supports bounded safe backspace, rejects wrong, control and extra characters before rendering them, and never writes a raw user-supplied byte. A malformed, stale or future namespace or a wrong first acknowledgement performs no random generation, pasteboard read or pasteboard clear. Only after that exact acknowledgement does it initially clear and verify the macOS general pasteboard, bind a fresh 128-bit nonce to the namespace through SHA-256 under a fixed domain, truncate the digest to 128 bits and print the resulting compact nonsecret challenge exactly as these three lines before its second prompt:
...
The rendered challenge line does not disclose the private namespace. Drag-select only all visible characters on that one line without either line break and transfer it with native macOS Command-C or Edit > Copy. A provider Copy control, Codex/browser-controller clipboard, injected clipboard write, AppleScript or scripted retyping is not the certified route. Without another Copy or paste action, manually type `VERIFY_F02_COPY_TEST_ONCE`. The command reads only `/usr/bin/pbpaste`, compares the complete challenge exactly without trimming or printing the observed value, rechecks namespace freshness after the read and finally clears and verifies the pasteboard on every normally settled post-START result. If a clipboard child cannot be proved reaped, it reports shutdown ambiguity without attempting a potentially racing clear; the attempt remains stopped for exact review. It never opens the Keychain, creates an operation lock or contacts a provider.

Success is only `STATUS=COMPLETE RESULT=F02_MACOS_PASTEBOARD_PREFLIGHT_VERIFIED_AND_CLEARED`. A malformed input, stale/future initial namespace, wrong first acknowledgement or random/dependency drift returns `F02_MACOS_PASTEBOARD_PREFLIGHT_INPUT_REJECTED` with exit `20`; a wrong second acknowledgement, read failure, mismatch or final freshness failure returns `F02_MACOS_PASTEBOARD_PREFLIGHT_ROUTE_REJECTED` with exit `21`; and an initial or final clear ambiguity overrides either result with `F02_MACOS_PASTEBOARD_PREFLIGHT_CLEAR_REJECTED` and exit `22`. These distinct nonzero codes let a direct-TTY launcher retain only the fixed public rejection class without redirecting or recording stdout; every unknown or signal exit remains unresolved and fail closed. A handled signal reports `F02_MACOS_PASTEBOARD_PREFLIGHT_INTERRUPTED`; unproved clipboard-child shutdown reports `F02_MACOS_PASTEBOARD_PREFLIGHT_SHUTDOWN_AMBIGUOUS`. After its sole terminal `STATUS` line has been emitted, a late signal is inert and cannot produce a second result. Every non-success closes readiness before credential creation and grants no retry or namespace reuse; a clear rejection additionally requires manual pasteboard review before any later work. Record only the fixed success result, its UTC time and a non-identifying route reference. Immediately initialize that same namespace after PASS; do not substitute or age it.

Only after that exact PASS, initialize the same namespace and bind the already approved window end in a direct managed pseudo-terminal:
...
Supply the public, nonsecret approved end only as exact UTC seconds `YYYY-MM-DDTHH:mm:ssZ` or canonical `YYYY-MM-DDTHH:mm:ss.000Z`. The first form is normalized to `.000Z`; offsets, local time, whitespace, lowercase `z`, nonzero or differently sized fractions, invalid dates and every other form are rejected. Before the operation lock, acknowledgement prompt, Keychain construction or any provider action, initialization requires the namespace start to be current within five minutes, the window to be active, and its duration to be at least one hour and no more than four hours. Only then type the fixed nonsecret acknowledgement `INITIALIZE_F02_KEYCHAIN_NAMESPACE_ONCE`. Initialization acquires the attempt's private OS advisory lock, proves that the namespaced service is empty, then rechecks the active window and five-minute namespace freshness immediately before its first write. It stores state `STAGING`, the canonical window end, then the exact namespace start last. Clipboard staging requires all three records to match. The start-last write is the admission fence: an interruption after either earlier write cannot admit private material, while the partial namespace remains eligible for the deletion fence and narrow cleanup. A malformed or initially invalid public-window rejection occurs before the lock or any Keychain/prompt action; elapsed-time rejection after the prompt still occurs before any Keychain write. After the outer lock is acquired, its fixed rejection identifies only the failed nonsecret stage—acknowledgement, dependency shape, namespace check, window recheck, state store, end store or start store—without exposing Keychain content. Shutdown ambiguity remains fail closed. Record the initialization PASS before console access or temporary credential creation. Keep the namespace in the private attempt record; it is an identifier, not a credential.

The initializer acknowledgement uses the same canonical-only visible reader as the two preflight acknowledgements. Store, generate, cleanup and every helper, operator, coordinator and retirement acknowledgement remain on their existing non-echoing readers. No credential, URL, identifier, HMAC value or other private input is eligible for visible rendering.
...
| Cloudflare token-retirement verifier | `LOAD_F02_TOKEN_RETIREMENT_KEYCHAIN_ONCE`, then `VERIFY_RETIRED_F02_TOKENS_ONCE` only after action-time console deletion | validates all retained inputs and provider-work closure before its claim or network access, detects the retained `W`, `R` or `W`+`R` role set, proves strict bounded active-success envelopes, remains running at `READY_F02_CLOUDFLARE_TOKEN_RETIREMENT`, then records only the exact role-matching HTTP `401` completion checkpoint without reading post-revocation content; any stop or ambiguity consumes the attempt and blocks namespace deletion |
...
After terminal rollback, cleanup and monitored all-off verification—but before deleting either token in the Cloudflare console—start the default-inert retirement verifier in a direct managed pseudo-terminal:

```sh
node scripts/verify-project2-f02-cloudflare-retirement.mjs \
  --execute-read-only \
...
Type `LOAD_F02_TOKEN_RETIREMENT_KEYCHAIN_ONCE`. The verifier holds the namespace advisory lock for its entire lifecycle. Before storing `claim.retirement-verifier` or making a network request, it validates the retained bundle state, window, detected `W`, `R` or `W`+`R` role set, credentials and required account/Queue identifiers. It proves every recorded generate, helper, operator, coordinator, rollback, rollback-recovery, cleanup, cleanup-recovery and lifecycle owner dead. If retained claims or checkpoints show provider work, it also requires the exact baseline rollback-completion checkpoint and, where deployment or cleanup work was claimed or recorded, the exact cleanup-completion checkpoint. It rechecks the owned lock and half-open closure cutoff immediately before storing the one-shot claim and immediately before every request; immediately after each request it rechecks the lock and exact claim.

For `W`, the active phase requires one token-verification HTTP `200`. For `R`, it requires HTTP `200` from token verification, main Queue metrics and DLQ metrics in that order. Each active response may contain at most 16,384 bytes and must decode as strict UTF-8 JSON. Token verification accepts only `success:true` with `result.status:"active"`; each Queue response accepts only `success:true` with nonnegative safe-integer `result.backlog_bytes`, `result.backlog_count` and `result.oldest_message_timestamp_ms`. The verifier uses fixed GET requests, refuses redirects, has a five-second timeout per request, never retries, retains no response content and prints no token, account or Queue identifier.

Only after every applicable active check does it emit `READY_F02_CLOUDFLARE_TOKEN_RETIREMENT`. Leave that exact process running. Delete each detected temporary credential once through the Cloudflare console, then return to the same terminal and type `VERIFY_RETIRED_F02_TOKENS_ONCE`. The verifier then requires exactly one HTTP `401` token-verification result for `W`; for `R`, it requires HTTP `401` from token verification, main Queue metrics and DLQ metrics, in that order. Those post-revocation checks inspect only the status. During that proof it never reads a response body or headers, never retries and never follows redirects. It stores the exact role-matching `checkpoint.retirement-complete` only after all required `401` results. Its only successful combined result is `F02_W_R_TEMPORARY_TOKENS_REVOKED_AND_UNUSABLE`; role-specific closure uses the corresponding fixed `W`-only or `R`-only result. The verifier does not create, broaden, revoke or delete a credential itself and grants no live authority. A failed active check or active-envelope validation, wrong confirmation, non-`401`, timeout, interruption, drift or ambiguous result grants no retry, leaves the one-shot claim without a completion checkpoint and deliberately blocks namespace deletion pending independent review and new authority.

The verifier's direct-terminal shutdown is fail-sticky. Handled `SIGINT`, `SIGTERM` or `SIGHUP`, prompt EOF/error/timeout, Ctrl-C/Ctrl-D or prompt I/O ambiguity aborts any in-flight authenticated request and proves every Keychain child scope zero-active before terminating the lock helper, retains the exact namespace marker and emits only `STATUS=STOPPED RESULT=F02_TOKEN_RETIREMENT_SHUTDOWN_AMBIGUOUS`. That redacted result is neither retirement proof nor retry authority. Do not remove or reuse the marker or namespace without independent exact process/provider review and separately authorized disposition.
...
Run it only after the terminal result or stop, required rollback, monitored all-off verification, completed retirement verifier, evidence retention and verifier-process exit are complete. Type `DELETE_F02_KEYCHAIN_NAMESPACE_ONCE`. The deletion utility first acquires the namespace advisory lock, before reading Keychain state, and holds the same verified lock inode through `deleteAll`'s namespace-empty proof. That excludes initialization, clipboard staging, private generation and every Keychain-mode provider actor for the whole deletion interval. It then proves that every recorded generate, helper, operator, coordinator, rollback, rollback-recovery, cleanup, cleanup-recovery and retirement-verifier PID owner is dead. If either retained token item exists, deletion also requires both the retirement claim and the exact role-matching completion checkpoint; if neither token item exists, both retirement records must be absent. Any recorded lifecycle, deploy or rollback boundary also requires an exact baseline rollback-completion checkpoint; any recorded deploy, candidate-deployed, clean-candidate or cleanup boundary additionally requires a cleanup-completion checkpoint. The utility advances the bundle to terminal `DELETION_STARTED`, rechecks every owner and closure checkpoint after that state fence, requires the fence and advisory lock still be present immediately before deletion, deletes every known item and verifies that the namespace is empty. Provider mutation hooks recheck both their expected bundle state and the held advisory lock. The state transition remains a terminal audit fence, not an atomic multi-item Keychain transaction; cross-process exclusion comes from the OS lock. An interrupted deletion may resume only from `DELETION_STARTED` after reacquiring that lock and repeating the same owner/closure checks; no provider action may resume. The cleanup action does not revoke a Cloudflare credential, delete a Worker version or delete retained evidence; those remain distinct closure controls.
