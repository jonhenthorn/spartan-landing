...
| `F-04` provider outage/recovery | Wrapped three-candidate causal runtime, guarded D1, dedicated operator and candidate-bound observer validators | Local `SQUARE_SEARCH_OUTAGE` → `APPS_FINALIZE_FAILURE` → `F04_OFFER_RECOVERY_ISOLATION` chain is ready; no candidate has been deployed, no reviewed F-04 provider-fixture action exists, and the available credential-blocked helper cannot substitute | **BLOCKED — separately reviewed exact new-customer fixture/credential path, Apps READY, deployment, exact-one-canary and Queues Read approval required** |
...
| recognized webhook replay | Wrapped exact-two ingress, controller, operator, observer and focused fixture/sender validators | Local profile-absent exact-two seed plus UUID-bound exact-webhook isolation/terminal primitive is ready | **NOT RUN — exact reserved fixture, separate permanent read-only Square 4xx-not-429 response gate, live deployment and Queues Read approval required** |
...
| `P-01` customer created/group add failed | Wrapped created-customer fault/recovery, guarded D1, dual-helper, dedicated operator and candidate-bound observer validators | Local two-candidate `SQUARE_GROUP_ADD_FAILURE` → `P01_GROUP_ADD_RECOVERY_ISOLATION` path is ready; no candidate has been deployed, no reviewed P-01 provider-fixture action exists, and the available credential-blocked helper cannot substitute | **BLOCKED — separately reviewed exact new-customer fixture/credential path, Apps READY, deployment, exact-one-canary and Queues Read approval required** |
...
No reviewed F-04 provider-fixture action exists. The current provider helper is compiled credential-blocked and cannot substitute. Result: `[BLOCKED — LOCAL THREE-CANDIDATE CAUSAL RUNTIME/OPERATOR/OBSERVER READY; SEPARATELY REVIEWED EXACT NEW-CUSTOMER FIXTURE/CREDENTIAL PATH, APPS READY, DEPLOYMENT, EXACT-ONE-CANARY AND QUEUES READ APPROVAL REQUIRED]`.
...
Local wrapper, SQL/CAS, helper, operator and observer suites make this mechanism local-ready. They do not show that either candidate was uploaded or deployed, that the case ran, or that Square/Apps changed. No reviewed P-01 provider-fixture action exists. The available provider helper is compiled credential-blocked and cannot substitute for that missing path. P-01 therefore remains live-blocked pending a separately reviewed exact new-customer fixture/credential path, Apps READY, deployment, exact-one-canary and Queues Read approval.

Result: `[BLOCKED — TWO-CANDIDATE P-01 IS LOCAL-READY; NO REVIEWED P-01 PROVIDER-FIXTURE ACTION EXISTS, AND FIXTURE/CREDENTIAL, APPS READY, DEPLOYMENT, EXACT-ONE-CANARY AND QUEUES READ GATES REMAIN]`.
...
The fixture builder, package reinspection and sender accept replay only when all three semantic gates hold: exact `refund.updated`; event ID `[A-Za-z0-9][A-Za-z0-9_-]{7,159}`; and object ID `SANDBOX_REFUND_CONFIRMED_ABSENT_[A-Z0-9]{8,64}`. The object prefix is a reserved procedural namespace, not proof of absence. Before preparing either Worker candidate, separately authorize one read-only Square sandbox GET for that exact object ID. Privately record a permanent `4xx` other than `429`; stop on `2xx`, `429`, `5xx`, network ambiguity or any other result. Do not proceed without both the syntax gate and this provider-response gate. A generic allowed `4xx` is rejection evidence, not proof that the object is absent.
...
Result: `[NOT RUN — EXACT-TWO REPLAY INGRESS AND EXACT-WEBHOOK PROCESSING PRIMITIVES IMPLEMENTED LOCALLY; RESERVED FIXTURE, SEPARATE PERMANENT READ-ONLY SQUARE 4XX-NOT-429 RESPONSE GATE, LIVE DEPLOYMENT AND QUEUES READ WINDOW NOT AUTHORIZED]`.
...
| `F-04` provider outage | — | local triple-helper + three-candidate causal runtime/operator/observer proof | `BLOCKED` | No reviewed F-04 provider-fixture action; separately reviewed exact new-customer fixture/credential path, Apps READY, exact-one-canary, deployment and Queues Read approval required; no live Square/Apps evidence |
...
| recognized webhook replay | — | local exact-two seed + exact-webhook isolation/observer proof | `NOT RUN` | Reserved fixture, separate permanent read-only Square 4xx-not-429 response gate, live deployment and Queues Read window not authorized |
...
| `P-01` customer/group partial | — | local dual-helper + two-candidate causal runtime/operator/observer proof | `BLOCKED` | No reviewed P-01 provider-fixture action; separately reviewed exact new-customer fixture/credential path, Apps READY, exact-one-canary, deployment and Queues Read approval required; no live Square/Apps evidence |
