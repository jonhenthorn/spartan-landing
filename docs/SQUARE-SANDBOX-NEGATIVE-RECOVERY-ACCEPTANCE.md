...
Status: **the first live F-02 attempt safe-stopped during its initial aggregate read-only D1 capture, before candidate traffic and before any request; no negative/recovery case in this worksheet has been accepted.** The separately completed core sandbox happy-path evidence remains valid but does not satisfy any row in this worksheet. The F-02 fixed closure was `F02_ZERO_REQUEST_SAFE_STOP_CLOSURE_CONFIRMED`, result `STOPPED`, HTTP `000`, requests `0`. The expired window and authority cannot be reused. Local validators now exercise the repaired fail-closed and recovery logic plus compiled-shut OAuth, provider-monitoring and backup/restore safety contracts, but local mocks are not Square, Google Apps Script, Cloudflare Queue, DLQ, R2 or restore acceptance. The deployed sandbox connector remains all-off, canary-only with an empty allowlist. This worksheet does not authorize a retry, deployment, flag change, webhook subscription, customer/order/refund fixture, credential entry or Queue/DLQ operation.

Three August 23 preparation/readiness attempts also closed before an F-02 candidate or request: one local package-read interruption, one Apps endpoint-isolation mismatch and one interrupted volatile synthetic-binding handoff. Their combined safe boundary was zero candidate versions, zero traffic assignments, zero consent/provider requests and verified revocation of every temporary credential; the recorded all-off baseline remained the sole 100% traffic assignment. These preparation stops are historical evidence only, not a case pass or reusable authority.
...
- Do not inspect or print secret values. Request drivers that need a private URL or signing material read it from hidden interactive prompts and keep it only in process memory. The F-02 coordinator instead pins the reviewed public sandbox origin in code, never prints it, and reads its private candidate UUID, synthetic submission ID and coupon only from hidden prompts. Every driver prints only its reviewed fixed status/result codes plus its documented bounded HTTP/request fields and, where applicable, bounded timing fields.
...
node scripts/validate-square-sandbox-f02-driver.mjs
node scripts/validate-square-sandbox-f02-pty.mjs
node scripts/validate-project2-f02-keychain.mjs
node scripts/validate-project2-f02-cloudflare-retirement.mjs
node scripts/validate-project2-f02-process-scope.mjs
...
node scripts/validate-square-provider-outcomes.mjs
node scripts/validate-square-ops-backup-restore-contract.mjs
...
- `scripts/run-square-sandbox-f02.mjs` plus `scripts/validate-square-sandbox-f02-driver.mjs` and `scripts/validate-square-sandbox-f02-pty.mjs` for the default-inert exact-one-request F-02 coordinator, mocked contract proof and offline true-pseudo-terminal rehearsal. The PTY validator runs the production coordinator main with its default hidden reader under an isolated environment, replaces every first side-effect boundary with fail-closed tripwires, supplies only valid-format dummy values plus a derived unequal confirmation, and requires an exact prompt-only zero-request transcript. Its forced parent-interrupt cleanup self-test signals and reaps the entire PTY process group and wrapper, handles stream failures and removes temporary state. The coverage signals both the top-level validator during self-test setup and the nested terminal parent. It requires Python 3.9 or newer on macOS or Linux and grants no live authority;
- `scripts/project2-f02-keychain.mjs`, `scripts/manage-project2-f02-keychain.mjs` and `scripts/validate-project2-f02-keychain.mjs` for the opt-in, attempt-namespaced macOS default-login-Keychain custody boundary. The mocked validator proves exact item labels, one-through-four-hour active-window binding, a derived bounded closure-claim cutoff, one-use claims, stdin-only Keychain writes, a fixed clipboard-clear attempt and verification with post-lock fail-sticky fencing, a shared depth-first reentrant OS advisory lock, durable nonempty-marker helper-death fencing, last-child shutdown reaping, redacted helper/coordinator results and verified namespace deletion without accessing a real Keychain or live provider. It verifies the fixed OS-account Application Support marker path is independent of `HOME`, `TMPDIR`, `TMP` and `TEMP`; uses disposable injected roots for tests; deterministically reproduces helper death during clipboard staging and main-process `SIGKILL` with a surviving detached child; then proves same-process and second-process cleanup exclusion with no dead-PID reclamation. The manual hidden-prompt modes remain the portable default, and neither mode grants live authority;
- `scripts/verify-project2-f02-cloudflare-retirement.mjs` and `scripts/validate-project2-f02-cloudflare-retirement.mjs` for the default-inert, direct-terminal retirement boundary. The mocked validator proves preclaim local-input and provider-closure admission, bounded active-success envelopes, continuous lock/claim/cutoff checks, action-time console handoff, the exact role-matched post-revocation HTTP `401` set, verified Keychain-child reaping, fail-sticky signal/EOF/error/timeout handling and deletion fencing without a live credential or provider call;
- `scripts/project2-f02-process-scope.mjs` and `scripts/validate-project2-f02-process-scope.mjs` for the Keychain-mode POSIX process-group and request-cancellation boundary. Its dummy parent/grandchild harness covers normal and nonzero exits, timeout, output limit, external and scope abort, sensitive-output rejection, direct-child reaping and full descendant-group termination without a provider call;
...
- `scripts/prepare-square-sandbox-provider-fixtures.mjs` plus `scripts/validate-square-sandbox-provider-fixtures.mjs` for five mocked provider-fixture shapes, three zero-mutation read-only preflights and a compiled fail-closed live credential gate. F-04/P-01 preflights perform only their exact private synthetic-customer searches; replay is bound to the freshly revalidated webhook package and may accept only an authorized target `400`–`499` other than `401`, `403` or `429`. Validation transport is confined to `https://provider-fixture.invalid`. The separate `manage-square-sandbox-oauth.mjs` local contract now validates two-role issuance, case scopes, callback state, full revocation and retirement proof against `.invalid` adapters. Its public path and the fixture helper both remain compiled shut: registered app IDs/digests, durable custody/admission, trusted time, callback/Square/case adapters and the reviewed integration are absent, while the fixture clients remain `null`. Every exact live execute action therefore still returns `CREDENTIAL_GATE_BLOCKED` before a prompt, package or request. See `SQUARE-SANDBOX-PROVIDER-FIXTURES.md`;
...
The checked-in sandbox entrypoint now contains a default-off, exact-target controller with P-02, F-04, P-01, O-01 and Q-01 causal state machines plus non-injecting offer/Queue isolation profiles, all documented in `SQUARE-SANDBOX-FAULT-HOOKS.md`. P-02's dedicated removal/recovery hook rejects the generic consumed-row injector. The one-time baseline migration deployed the then-reviewed default-off controller build only as the current all-off sandbox baseline; the newer F-02 and P-02 observer repairs in the reviewed repository source are code-only and are not deployed. No live-case candidate, control profile, canary or temporary control is active or armed, and no live case request has run. The first F-02 attempt nevertheless reached its initial aggregate read-only D1 capture and then closed `STOPPED` with `REQUESTS=0`; do not describe that bounded attempt as unrun or accepted. The local request drivers are inert without `--execute`; they do not add a Worker route or test hook. The Worker-version operator is likewise inert without an exact execute action plus its complete fixed acknowledgement vector; `--plan` and `--check` cannot mutate state. Every current controller profile has a dedicated candidate action; the retained generic prepare/deploy compatibility vectors have empty mode sets and cannot reach private inputs or a process. The exact-one, exact-two replay and exact-two O-01 seeds; F-04 search/Apps/recovery chain; dedicated injecting offer/Queue profiles; P-01 fault/recovery; offer/redrive/replay/P-02/O-01/Q-01 isolation; rollback; and cleanup actions still require a fresh separately approved live window. The DLQ helper is also local-only and inert without an explicit inspect or redrive mode; its account-scoped Queues Write credential and every Queue action remain separate live approvals. The older deliberate crash controls in `scripts/validate-square-connector.mjs` remain in-memory test behavior only.

On August 22, 2026, offline repair replaced the over-limit observer patterns and proved the full F-02 aggregate query against the pinned local D1 runtime. The separate P-02 aggregate observer was also repaired to avoid the local D1 expression-depth ceiling by materializing its lineage input and comparing the Apps payload as one exact vector; its 47 integer fields and three JSON buckets compile and execute under the same pinned local runtime. These are offline validation results only. Neither repair is deployed, neither is live-case evidence and neither grants retry, credential, traffic, request, provider, Apps, Queue, D1 or production authority.
...
Require the provider-fixture validator's five mocked mutation cases and three `.invalid`-only read-only cases, plus the OAuth lifecycle validator's local-contract-only result, exact inert `NO_REQUEST` and exact live-boundary `CREDENTIAL_GATE_BLOCKED` results. Local successes are contract tests only; no live `F04_NEW_CUSTOMER_SLOT_CLEAR`, `P01_NEW_CUSTOMER_SLOT_CLEAR` or `REPLAY_PERMANENT_SQUARE_REJECTION_READY` evidence exists. Until registered dedicated apps, durable custody/admission, trusted time, exact callback/Square/case adapters and reviewed fixture integration clear both compiled gates, the seed action's `--ack-exact-fixture-ready` cannot truthfully be supplied for P-02/Q-01/Q-02 and the new preflights cannot clear F-04/P-01/replay. Every provider-dependent live path remains blocked or not run as marked below.
...
The audited live source was a legacy all-off sandbox version whose only permitted metadata difference from the checked current all-off configuration was that `SQUARE_SANDBOX_FAULTS_ENABLED` was absent instead of exactly `"false"`. During the owner-approved August 21, 2026 preparation-only window of 14:00–14:30 UTC, one exact current all-off target was uploaded at 14:14:35 UTC. The command returned `STATUS=REJECTED RESULT=TARGET_PREPARE_REJECTED_LEGACY_TRAFFIC_CONFIRMED` and was not retried. Independent post-upload and readiness checks confirmed that one retained in-window target was inactive at 0% and the exact source was the sole 100% allocation; the separate read-only check returned `STATUS=READY RESULT=READY_SANDBOX_LEGACY_TO_CURRENT_ALL_OFF_MIGRATION`.

In the separately approved August 21, 2026, 8:41–9:11 p.m. Central final-deployment window, the exact retained current all-off target became the sole 100% sandbox baseline. The fixed migration result, strict checks before and after credential revocation, new observer baseline and monitored all-off proof all passed. Both historical versions were retained, both Queues stayed empty, the temporary Queues Read credential was deleted and independently rejected by its verification and both Queue endpoints, and the independent reviewer, evidence custodian and business owner closed the migration `PASS`. No sandbox case, provider request, secret change, Queue or D1 write, Apps action, Brevo action or production action occurred.

Pinned Wrangler `4.124.0` output/parser review identified the false-negative cause: successful upload output contains a D1 `preview_database_id` UUID before the labeled `Worker Version ID`, while the historical parser required exactly one UUID in the full output. Because the historical catch collapsed the inner failure code, this is a reproducible output/parser-contract finding, not a claim that the terminal rejection exposed its inner error. The reviewed parser now reads the unique labeled Worker version ID; the historical result remains rejected.

The following three-surface procedure is retained as the completed historical control record. Do not rerun it for the retained August 21 target:

1. Preserve the August 21 rejection and upload evidence without retrying preparation. In a private copy of the migration record, require combined command/session and version evidence identifying the one in-window upload and no second in-window upload, recorded checkpoints showing the retained exact target inactive and the exact source as the sole 100% allocation, scoped independent reviewer signoff and the separate readiness result. The owner must explicitly record `RETAINED_PREPARATION_EXCEPTION_ACCEPTED` or reject the retained target. That record-only label is not `PREPARED`, final deployment `GO` or case authority.
2. For a new unexpired final-deployment window, rerun `--check-legacy-baseline-migration`; require a fresh `STATUS=READY RESULT=READY_SANDBOX_LEGACY_TO_CURRENT_ALL_OFF_MIGRATION` after exact source, target, account, commit and traffic revalidation.
3. Independently reconfirm empty reported Queues, zero nonterminal webhook/outbox work, disabled Square sandbox webhook subscription, quiet ingress and no case/provider request. Obtain a separate final owner `GO`, then deploy only with `--execute --migrate-legacy-baseline-to-current-all-off`; require `STATUS=COMPLETE RESULT=SANDBOX_LEGACY_TO_CURRENT_ALL_OFF_MIGRATION_CONFIRMED BASELINE_VERSION=<target-uuid>`.

Keep the account, commit and both version IDs in the private owner evidence record and hidden prompts only. The target differs from the legacy source only by adding the explicit false fault flag; every other handler, variable, resource and standing-secret name must match. Preparation changes no traffic or secret value. The reviewed one-upload bounded-convergence behavior is limited to the same invocation after it parses one distinct uploaded target UUID: only a transient `VERSION_METADATA_UNAVAILABLE` or `TRAFFIC_STATUS_UNAVAILABLE` may trigger one exact immutable reread and normal `PREPARED`; semantic drift never converges. It is not a later retained-target reread mode and cannot overwrite the preserved historical rejection. Deployment changes only the sandbox traffic allocation. An ambiguous deploy rolls back only to the exact legacy source; a rejection, `ROLLBACK_UNCONFIRMED`, drift or unexpected allocation is a stop and never case evidence.

If the operator process is interrupted after migration deployment may have started, or the attempt is otherwise ambiguous without a fixed terminal result, do not infer state or rerun the migration. Under the separately preauthorized migration-recovery authority, use only `--execute --recover-interrupted-legacy-baseline-migration` with the exact acknowledgement vector and hidden account/source/target prompt order in [Standalone interrupted-migration recovery](SQUARE-SANDBOX-FAULT-HOOKS.md#standalone-interrupted-migration-recovery). It admits only the exact legacy source or exact prepared target at 100% traffic. Exact target-to-source restoration returns `STATUS=COMPLETE RESULT=EXACT_LEGACY_MIGRATION_RECOVERY_CONFIRMED`; an already exact source returns `STATUS=COMPLETE RESULT=LEGACY_MIGRATION_RECOVERY_ALREADY_AT_EXACT_SOURCE` without a traffic change. Metadata drift, split allocation, a third version or any unconfirmed recovery fails closed as `STATUS=REJECTED RESULT=LEGACY_MIGRATION_RECOVERY_UNCONFIRMED`; stop and obtain an independent read-only review because that result does not establish the final allocation.

Standalone recovery is not case approval, migration success or proof of the current all-off baseline. No standalone recovery was required for the completed August 21 migration. The first F-02 attempt is retained as a zero-request `STOPPED` result, not a pass. Any F-02 retry remains `NOT APPROVED` and requires a fresh activation record, exact reviewed commit, unexpired window and final `GO`; the expired record and authority cannot be reopened or reused.
...
| --- | --- | --- | --- |
| one-time all-off baseline migration | Exact legacy-source/current-target metadata and three-surface operator validation | Fixed migration success, strict checks, new observer baseline, monitored all-off stability, both-version retention, credential revocation and all closure signatures are privately retained | **PASS — closed August 21, 2026; grants no case or production authority** |
...
| `F-02` declined consent | Wrapped offer-isolation, exact-one-request coordinator, candidate-bound observer, connector and direct-PTY validators | First approved attempt safe-stopped in its initial aggregate read-only D1 capture with `F02_ZERO_REQUEST_SAFE_STOP_CLOSURE_CONFIRMED`, `HTTP=000`, `REQUESTS=0`; no candidate traffic or request occurred. Offline query-limit repair passes the pinned local D1 runtime, and the wrong-confirmation true-PTY rehearsal passes without side effects | **STOPPED; RETRY NOT APPROVED — the historical result remains `STOPPED` and unaccepted, the PTY result is local launch-boundary proof only, and the expired authority cannot be reused; a fresh activation record, exact reviewed commit, exact-one-canary window, deployment and Queues Read approval are required** |
...
This is the only negative case that can be rejected before Turnstile and every provider call, but the Worker checks its complete offer configuration before it checks consent. The local fixed-mode `OFFER_ROUTE_ISOLATION` candidate now supplies that exact boundary: `SQUARE_SANDBOX_CONTROL_PROFILE=OFFER_ROUTE_ISOLATION` matches the encrypted mode, `SQUARE_SANDBOX_FAULTS_ENABLED=false`, offer/pass/webhook/consumer/owner-harness are true, reconciliation is false, and exactly one valid canary is allowlisted. The sandbox wrapper admits only query-free owner-harness GET and an offer POST carrying that exact configured canary; it rejects a non-canary POST before consent and blocks pass/webhook/config/other fetch, Queue and scheduled invocation. This preflight exists only in the sandbox wrapper. The common production offer route keeps its existing consent-before-canary order, avoiding a production submission-membership oracle. The isolation never injects or consumes a control row. Use only the dedicated helper and operator actions in `SQUARE-SANDBOX-FAULT-HOOKS.md`; do not assemble or deploy it manually. The complete local wrapped test proves consent `no` stops before Turnstile, Apps, Square, D1 or Queue, but is not live acceptance.

F-03 and R-01 continue to use the standalone observer handoff. F-02 now requires the separate one-process coordinator so a stable zero-delta interval cannot pass when no request occurred. While the current all-off baseline still owns traffic, start this default-inert F-02 coordinator before deployment:
...
The hidden prompt order is exact candidate UUID, private approved synthetic submission ID, private approved synthetic coupon code and fixed confirmation `RUN_F02_DECLINED_CONSENT_ONCE`. The coordinator captures a fresh aggregate all-off baseline and requires fixed `READY_OFFER_ISOLATION_DEPLOY_QUEUES_REPORTED_EMPTY` before the operator may supply `--ack-ready-offer-isolation-deploy-queues-reported-empty` to the dedicated deploy action. After the exact candidate reaches 100%, it emits `READY_F02_ONE_REQUEST_CANDIDATE_ACTIVE`, compares the hidden submission ID to the remotely verified candidate canary, sends one fixed POST to the fixed sandbox origin with no query, redirect or retry, requires exact HTTP `400` and exact two-field `CONSENT_REQUIRED` JSON, and emits `OBSERVED_F02_REQUEST_COMPLETION_HANDSHAKE`. Only then does it perform the stable zero-delta and full read-only terminal checks.

Use a direct managed pseudo-terminal only, with one private value manually supplied after each observed prompt. Expect, pexpect, Tcl, AppleScript, browser/UI automation, pipes, heredocs and shell-generated prompt matchers are prohibited; do not automate or supply live prompt values through arguments, environment variables or files. Before a fresh activation record can receive final `GO`, `node scripts/validate-square-sandbox-f02-pty.mjs`, `node scripts/validate-project2-f02-keychain.mjs`, `node scripts/validate-project2-f02-cloudflare-retirement.mjs` and `node scripts/validate-project2-f02-process-scope.mjs` must run from the exact reviewed commit and pass. The PTY rehearsal exercises the production coordinator main and default hidden reader under an isolated pseudo-terminal while replacing every first side-effect boundary with fail-closed tripwires. PASS requires four exact ordered prompts, no input echo, one fixed HTTP `000` / requests `0` rejection, no other terminal or readiness output, and zero capture, watcher, fetch, checkpoint or request-attempt calls. Its forced parent-interrupt cleanup self-test must signal and reap the entire PTY process group and wrapper, handle stream failures and remove temporary state. The coverage signals both the top-level validator during self-test setup and the nested terminal parent. Python 3.9 or newer on macOS or Linux is required. Each rehearsal is local proof only; it is not case readiness and grants no credential, candidate, traffic or request authority.

The F-02 request itself performs no Turnstile, provider, Apps or Square call and no Queue or D1 mutation. Its coordinator separately performs the approved aggregate read-only Queue and D1 checks before the request and after the completion handshake. Keep the raw coordinator/operator/Wrangler transcript and candidate/rollback outputs private because adjacent actions may emit operational UUIDs. Shared evidence may contain only sanitized fixed checkpoints/results, HTTP/request counts, bounded UTC times and aggregate read-only zero-delta evidence.

The opt-in Keychain F-02 path is one process after candidate preparation, not an external deployment handoff. It atomically owns `claim.lifecycle` as `COORDINATOR:PID:<pid>`, holds the reviewed baseline at 100% and the unpublished candidate at 0% through readiness, and internally performs the exact deploy only after the owner supplies FINAL GO phrase `RUN_F02_DECLINED_CONSENT_ONCE`. Its candidate-bound watcher is already running; the Keychain path performs no external deployment polling. After the terminal result or ordinary stop, the same coordinator performs normal exact rollback, any required baseline-only cleanup and monitored closure verification in-process. Rollback does not always require a prior deploy claim: before deployment it is the exact baseline no-op/drift closure, while cleanup runs only if durable claims show deployment may have occurred.

When the in-process deployment callback returns, the watcher treats the candidate as already observed; a later baseline or third-version poll is terminal alternation and cannot reopen a wait. Immediately before the sole transport, the coordinator first durably consumes the no-retry request slot, then performs one fresh aggregate read-only exact candidate-at-100%, canary, Queue, D1 and topology verification, and finally rechecks the active window. Drift or exact-endpoint expiry sends zero requests while retaining the consumed slot and requiring rollback/cleanup; it never grants a retry.

The Keychain attempt requires a one-through-four-hour owner-approved window. Its immutable half-open closure-claim cutoff is `window end + approved window duration`. After the case window expires, a new rollback, recovery, cleanup or retirement-verification action may be claimed only while the clock is strictly before that cutoff, through its durable lifecycle/rollback/cleanup/retirement claims and exact remote candidate-or-baseline state. The operator rechecks time after state/liveness review, immediately before every action claim and immediately before every provider mutation or read-only retirement request. A provider request started before the cutoff may settle afterward, but no later request, mutation or retry may start. At or after the cutoff, no new close, recovery or retirement-verification claim is admitted without fresh owner authority and exact-state review. A new process must never retry the coordinator or invoke normal rollback. Candidate-deployment ambiguity does not trigger a second deployment inside the deploy boundary; the coordinator's separately claimed rollback owns the sole baseline-restoration attempt. Standalone recovery is only for a proved-dead `COORDINATOR:PID:<pid>` or `ROLLBACK:PID:<pid>` lifecycle owner and must use exactly:
...
That recovery selector accepts only the durable attempt baseline/candidate claims and exact remote traffic wholly at the named baseline or candidate. If a cleanup owner died, it also requires the prior exact rollback checkpoint, claims `claim.cleanup-recovery` and may admit only the exact `derived.cleanup-candidate-version` recorded after clean metadata and baseline-traffic verification. A cleanup crash before that clean-candidate claim admits no intermediate upload. The selector confirms or restores only the exact baseline at 100%; a live owner, split traffic, unrecorded version or claim drift fails closed. It never authorizes a request retry. Successful ordinary recovery permits later cleanup only from the durable completed-rollback claim plus fresh exact-baseline verification; successful cleanup recovery records baseline cleanup completion. Recovery is itself one-shot: if it dies or is ambiguous after its durable claim, the attempt cannot take over, self-retry or perform more provider work. Preserve its namespace and evidence; later intervention requires new owner authority and independent exact-state review. The portable manual hidden-prompt procedure remains distinct: it retains the documented separate operator deployment, rollback and cleanup handoffs and does not use the Keychain lifecycle lease.

After terminal closure and credential revocation, Keychain namespace deletion first acquires the same namespace-wide OS advisory lock used by initialization, clipboard staging, the helper, every operator boundary, the coordinator, rollback, cleanup and the retirement verifier. Its fixed default root is `<OS-account-home>/Library/Application Support/com.spartan.project2.f02/namespace-operation-locks-v2`, resolved from the operating-system user record rather than `HOME` or any temporary-directory variable. Each action's exact-`0600` file carries a nonsecret `MAIN:<pid>:ACTION:<128-bit nonce>` marker. Verified normal release overwrites that marker with the nonce-bound nonempty `RELEASED:<128-bit nonce>` tombstone and fsyncs it while `flock` remains held; after the helper exits cleanly and is reaped, the parent verifies the exact root, owner, mode, link count, same inode and tombstone, unlinks that one file and proves the path absent. Any retained active marker or `RELEASED` tombstone is rejected regardless of recorded-PID liveness, so unexpected helper death, unproved cleanup, handled interruption or `SIGKILL` blocks deletion even after the original main process dies. Such a marker requires independent exact descendant/process/provider/traffic review and separately authorized exact marker disposition before deletion can acquire the lock. The fixed path is a security compatibility invariant and may not be renamed or version-bumped without an explicit migration/disposition plan; deliberate same-account subtree replacement, disk loss and restore rollback remain outside the containment claim. Deletion then holds the verified lock inode through the namespace-empty proof, so those actions cannot overlap it; proves every generate/helper/operator/coordinator/rollback/rollback-recovery/cleanup/cleanup-recovery/retirement-verifier PID owner dead; requires the applicable rollback and cleanup completion checkpoints; and, while either retained token exists, requires `claim.retirement-verifier` plus the exact W-only, R-only or W+R role-matching `checkpoint.retirement-complete`. If neither token exists, both retirement records must be absent. It then enters terminal `DELETION_STARTED` and rechecks those conditions and that state immediately before deletion. Provider mutation hooks recheck both their loaded state and held lock. The state remains an audit fence rather than an atomic multi-item transaction; cross-process exclusion comes from the OS advisory lock plus the durable fail-sticky marker, and an interrupted deletion can only resume deletion after the same fence review, never provider work.

On handled `SIGINT`, `SIGTERM` or `SIGHUP`, require exactly one fixed line with `HTTP=000`: `STOP_F02_DRIVER_INTERRUPTED` only when every registered descendant and the advisory-lock helper are proved reaped, otherwise `STOP_F02_DRIVER_SHUTDOWN_AMBIGUOUS`. Either line may report `REQUESTS=0` only before the durable request-attempt reservation; afterward `REQUESTS=1` means the sole request slot was consumed and a fetch may have been attempted. On a stop, that conservative marker is not proof that a network call occurred. Only the request-completion handshake `OBSERVED_F02_REQUEST_COMPLETION_HANDSHAKE` together with terminal PASS proves the one request. Signal handling aborts the in-process fetch through its scoped signal and initiates bounded reaping of active coordinator descendant process groups, including authenticated Wrangler and observer children. Normal shutdown requires proof that the direct child was reaped, the group is gone and the active registry is empty; any false, thrown or nonzero termination proof remains STOP/ambiguity and does not exit through the signal handler. No signal path sends the nonce-bound normal release: the durable marker survives main-process exit and blocks automated recovery. Process death cannot perform provider rollback. The interrupted coordinator performs no retry. A fixed interrupt result, an unhandled termination or any exit without a fixed terminal line is not acceptance or retry authority; never run the normal coordinator path again. Before using the exact dead-owner recovery selector when the candidate may be active, require independent exact descendant/process/provider/traffic review and separately authorized exact marker disposition.

Expected terminal output for a future successful attempt remains `STATUS=COMPLETE RESULT=PASS_F02_CANARY_DECLINED_CONSENT_NO_LOCAL_DELTA HTTP=400 REQUESTS=1`. Require stable monitored zero local delta, both Queues reported empty at baseline and post-request terminal, the confirmed one-request handshake and no D1 claim/pass row, Apps consent/link/event write, Square request or Queue message. Provider and Apps activity remain labeled `NOT_OBSERVED`; the route ordering and zero local delta are the bounded pre-provider proof. The exact canary/coupon pair remains private ledger evidence; the canary gate does not prove coupon-to-claim association. Any unexpected response, second callback, `STOP`, timeout, candidate change or sent-but-unconfirmed transport is ambiguous: do not retry, and immediately use the exact rollback and cleanup path. A manual browser request and standalone `watch-offer-isolation F02` cannot satisfy acceptance. Historical result: `[STOPPED — F02_ZERO_REQUEST_SAFE_STOP_CLOSURE_CONFIRMED; HTTP 000; REQUESTS 0; CANDIDATE TRAFFIC AND REQUEST NOT REACHED]`. Current decision: `[RETRY NOT APPROVED — A FRESH ACTIVATION RECORD, EXACT REVIEWED COMMIT, UNEXPIRED EXACT-ONE-CANARY WINDOW, DEPLOYMENT AND QUEUES READ CREDENTIAL ARE REQUIRED]`.

The earlier `OBSERVED_F02_DECLINED_CONSENT_NO_LOCAL_DELTA_STABLE` result is retained only as retired historical diagnostic evidence. It did not causally attest that a request occurred and must not be accepted for a current F-02 case. The new `PASS_` result is intentionally a different contract and cannot be backfilled onto an old observation.
...
For opt-in Keychain F-02, those two normal close steps are coordinator-internal and are not separate external operator handoffs. The durable lifecycle owner and attempt claims preserve only exact rollback/cleanup authority through the derived half-open closure-claim cutoff `window end + approved window duration`; they create no new case authority. The clock is rechecked before every provider mutation. A provider request started before the cutoff may settle afterward, but at or after it no new rollback, recovery, cleanup mutation or retry is valid without fresh owner authority and exact-state review. If that process dies, use only the exact standalone dead-owner recovery command in the F-02 section, never the normal retry or ordinary rollback path. The legacy/manual procedures continue to use their documented separate close actions.

The fixed `OFFER_ROUTE_ISOLATION` candidate is part of this driver-managed rollback contract. Start its exact case-bound controller under the original baseline and require `READY_OFFER_ISOLATION_DEPLOY_QUEUES_REPORTED_EMPTY` before deployment. After F-02 terminal `PASS_F02_CANARY_DECLINED_CONSENT_NO_LOCAL_DELTA`, the F-03 checkpoint/repeat and terminal `PASS_F03_AMBIGUOUS_MATCH_REPEAT_NO_SECOND_DELTA`, or the R-01 terminal `PASS_R01_READY_REPLAY_ONE_FRESH_PASS`, use exact rollback immediately from that named candidate, then baseline-only cleanup. F-02 transport ambiguity is never retry authority. Any watcher `STOP`, timeout, drift or ambiguity takes the same rollback path; `READY_` and F-03's intermediate `OBSERVED_` are never terminal acceptance. A separately approved normal non-controller run would use a different controlled drain: disable offer, pass and owner harness first; empty the canary allowlist; keep verified webhook/consumer processing only long enough to drain accepted synthetic work; turn reconciliation off; confirm no unintended nonterminal work; disable the Square webhook subscription; then turn webhook and consumer off. Do not invoke the controller-window rollback after arbitrary intermediate versions.
...
| `F-02` declined consent | 2026-08-22 UTC | live attempt closed before candidate traffic/request; repaired coordinator/observer validated against pinned local D1; offline true-PTY wrong-confirmation rehearsal passed | `STOPPED; RETRY NOT APPROVED` | `F02_ZERO_REQUEST_SAFE_STOP_CLOSURE_CONFIRMED`; HTTP `000`; requests `0`; all-off cleanup and credential revocation closed; PTY rehearsal is local launch-boundary proof only; expired authority cannot be reused; repairs are not deployed and a fresh activation record/exact reviewed commit/window/final `GO` are required |
