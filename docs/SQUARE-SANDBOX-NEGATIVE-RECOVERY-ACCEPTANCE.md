...
The Keychain attempt requires a one-through-four-hour owner-approved window. Its immutable half-open closure-claim cutoff is `window end + approved window duration`. After the case window expires, one rollback, recovery or cleanup action may be claimed only while the clock is strictly before that cutoff, through its durable lifecycle/rollback/cleanup claims and exact remote candidate-or-baseline state. The operator rechecks time after state/liveness review, immediately before every action claim and immediately before every provider mutation. A provider request started before the cutoff may settle afterward, but no later mutation or retry may start. At or after the cutoff, no new close or recovery claim is admitted without fresh owner authority and exact-state review. A new process must never retry the coordinator or invoke normal rollback. Candidate-deployment ambiguity does not trigger a second deployment inside the deploy boundary; the coordinator's separately claimed rollback owns the sole baseline-restoration attempt. Standalone recovery is only for a proved-dead `COORDINATOR:PID:<pid>` or `ROLLBACK:PID:<pid>` lifecycle owner and must use exactly:
