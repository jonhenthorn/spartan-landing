...
| `F-02` declined consent | Wrapped offer-isolation, exact-one-request coordinator, candidate-bound observer and connector validators | First approved attempt safe-stopped in its initial aggregate read-only D1 capture with `F02_ZERO_REQUEST_SAFE_STOP_CLOSURE_CONFIRMED`, `HTTP=000`, `REQUESTS=0`; no candidate traffic or request occurred. Offline query-limit repair now passes the pinned local D1 runtime | **STOPPED; RETRY NOT APPROVED — the historical result remains `STOPPED` and unaccepted, and the expired authority cannot be reused; a fresh activation record, exact reviewed commit, exact-one-canary window, deployment and Queues Read approval are required** |
...
| `F-02` declined consent | 2026-08-22 UTC | live attempt closed before candidate traffic/request; repaired coordinator/observer validated only against pinned local D1 | `STOPPED; RETRY NOT APPROVED` | `F02_ZERO_REQUEST_SAFE_STOP_CLOSURE_CONFIRMED`; HTTP `000`; requests `0`; all-off cleanup and credential revocation closed; expired authority cannot be reused; repairs are not deployed and a fresh activation record/exact reviewed commit/window/final `GO` are required |
