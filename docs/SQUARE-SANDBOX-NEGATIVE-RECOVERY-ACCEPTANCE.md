...
Standalone recovery is not case approval, migration success or proof of the current all-off baseline. No standalone recovery was required for the completed August 21 migration. F-02 remains `NOT APPROVED` and requires its separate activation record even though the migration, strict checks, monitored all-off proof, credential revocation, independent review and owner closure are complete.
...
| `F-02` declined consent | Wrapped offer-isolation, exact-one-request coordinator, candidate-bound observer and connector validators | Local fixed-mode helper/operator candidate plus in-memory sender/watcher handshake admits only the exact canary request; the request path performs no Turnstile/provider/Apps/Square call or Queue/D1 mutation, while the coordinator separately proves aggregate read-only pre/post zero delta | **NOT APPROVED — baseline migration is closed; a separate F-02 activation record, exact-one-canary window, deployment and Queues Read approval are still required** |
...
Expected terminal output: `STATUS=COMPLETE RESULT=PASS_F02_CANARY_DECLINED_CONSENT_NO_LOCAL_DELTA HTTP=400 REQUESTS=1`. Require stable monitored zero local delta, both Queues reported empty at baseline and post-request terminal, the confirmed one-request handshake and no D1 claim/pass row, Apps consent/link/event write, Square request or Queue message. Provider and Apps activity remain labeled `NOT_OBSERVED`; the route ordering and zero local delta are the bounded pre-provider proof. The exact canary/coupon pair remains private ledger evidence; the canary gate does not prove coupon-to-claim association. Any unexpected response, second callback, `STOP`, timeout, candidate change or sent-but-unconfirmed transport is ambiguous: do not retry, and immediately use the exact rollback and cleanup path. A manual browser request and standalone `watch-offer-isolation F02` cannot satisfy acceptance. Result: `[NOT APPROVED — BASELINE MIGRATION IS CLOSED, BUT IT DOES NOT APPROVE THE F-02 ACTIVATION RECORD, EXACT-ONE-CANARY WINDOW, DEPLOYMENT OR QUEUES READ CREDENTIAL]`.
...
| `F-02` declined consent | — | local offer-isolation + candidate-bound exact-one-request coordinator/observer primitive ready | `NOT APPROVED` | Baseline migration closed `PASS`; it does not approve the later F-02 activation record, exact-one-canary window, deployment or Queues Read credential; no live request has run |
