...
| `F-02` declined consent | Wrapped offer-isolation, exact-one-request coordinator, candidate-bound observer and connector validators | Local fixed-mode helper/operator candidate plus in-memory sender/watcher handshake admits only the exact canary request and proves post-request zero local delta without Turnstile/provider/D1/Queue work | **NOT APPROVED — baseline migration is closed; a separate F-02 activation record, exact-one-canary window, deployment and Queues Read approval are still required** |
