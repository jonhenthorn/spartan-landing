...
On handled `SIGINT` or `SIGTERM`, require exactly one `STOP_F02_DRIVER_INTERRUPTED` terminal line with `HTTP=000`. It may report `REQUESTS=0` only before the fetch-attempt marker; from immediately before the single fetch invocation onward it must report `REQUESTS=1`. The interrupted coordinator performs no retry and no automatic rollback. A fixed interrupt result, an unhandled termination or any exit without a fixed terminal line is not acceptance or retry authority; immediately use the separate exact rollback and cleanup path if the candidate may be active.
