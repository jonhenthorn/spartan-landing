...
Status: **the bounded D1 monitor is remotely proven in the isolated sandbox, and the counts-only alert engine plus read-only Queue/DLQ source remain deployed inertly on schema `3`. A schema-4 signed Apps Script health source is locally implemented and validated only; migration `0004`, its URL/secret and its Worker version are not deployed. Every live capability flag is false; there is no Queue token, Apps health secret/URL, alert sender/binding, recurring backup or restore automation.** Project 2 production activation remains blocked.
...
- Monitoring runs only when its flag is true and the exact five-minute cron fires. The schema-4 candidate requires a separate source flag for Queue metrics and another for Apps health; either source flag requires aggregate monitoring. Queue access additionally requires three exact resource IDs and a deploy-only Queues Read token. Apps access requires one exact Google deployment URL, a dedicated deploy-only health secret and reviewed expected states. The deployed but disabled alert engine can run only on that same cron, requires monitoring and two distinct role bindings plus a bounded sender. A wrong or missing cron touches none of those bindings or credentials. Backup and restore flags remain fail-closed because those lanes are not implemented.
- Migrations `0001`, `0002` and `0003` are applied to both sandbox operations databases. Runtime preserved eight monitor runs and two resolved incidents with zero active incidents; preview remained empty; both have zero delivery, backup and restore rows and zero orphan deliveries. Remote inspection found all 27 delivery columns, both reviewed indexes, all three Queue alert pairs and no foreign-key failures. Pre/post runtime exports matched exactly for monitor/incident data and the post copy passed SQLite integrity and foreign-key checks. The sandbox intentionally omits R2 because that account feature and the backup lane are not approved; production retains the future placeholder.
- Permanent sandbox Worker version `29ab2f6c-265f-4542-81ec-a4dbf41f2a0b` is bound to runtime operations D1 `2e2fc9f6-0a81-453b-9af6-8d4104965f8e` and connector D1 `9531221e-cabe-4ed4-b7d4-f715798b8945`, has only a scheduled handler, schema `3`, no secret/Queue/email/R2 binding and every capability false. The five-minute trigger after deployment left all counts and prior update timestamps unchanged, including zero alert deliveries.
...
- No checked or deployed configuration contains an email binding, sender, recipient, Queue binding/token, Apps health URL or Apps health secret. The Queue metrics account/main/DLQ IDs and Apps expected states are non-secret reviewed configuration only; both source flags are false. No connector configuration, Queue message, webhook, Apps Script property or production account changed during the inert schema-3 rollout or this local schema-4 implementation.
...
The Apps inspection uses Google Spreadsheet metadata reads, including the reviewed ledger schema/format checks, and batches each ledger tab's format scan into one Spreadsheet read. Before activation, prove both cold and warm calls finish inside the shared five-second deadline on the isolated sandbox ledger. A timeout is an unavailable signal, never a healthy result; if live latency is not reliable, narrow the reviewed metadata inspection or make a separately reviewed timeout change before enabling the source.
...
Not yet remotely covered by this slice: enabled Cloudflare Queue/DLQ depth, external credential/provider health, Apps Script probing, live external alert delivery, backup/restore freshness, or ledger/group comparison beyond the bounded connector codes. Queue and Apps source code and local tests are complete, but Apps deployment/migration/secret/URL/request evidence and Queue token/request evidence remain separate gates. A monitor run is never `HEALTHY` when a configured source is unavailable.
...
- [x] Migration `0003` is applied to preview then runtime with preserved counts/indexes, zero orphans and integrity/foreign-key proof; the schema-3 Worker is deployed with every flag false and one scheduled interval proved zero writes and zero network access.
...
- [ ] Publish and verify the matching Apps Script health contract first; then apply migration `0004` to preview and runtime with export/integrity/preservation proof. Install a new dedicated sandbox URL/secret outside Git, prove healthy/disabled/failure/mismatch/recovery behavior, and return `OPS_APPS_SCRIPT_MONITORING_ENABLED=false`. None of those live steps is approved or complete yet.
...
- [x] The Apps health, Apps Script, operations, connector, frontend, form-backend and site validators plus both Wrangler dry-runs pass for the local schema-4 candidate. Dry-run packaging is not deployment or live proof.
...
The operations plane is done only when it can detect and externally report the required connector failures, maintain verified private backups, prove isolated restoration and clean rollback, all without PII duplication or a public surface. Every alert and backup must have age, delivery/integrity and recovery evidence. Default-off must remain a tested zero-operation state. The remote proof approves only the inert schema-3 sandbox deployment; the local schema-4 candidate does not approve an Apps deployment/migration/secret/URL/request, Queue token/request, email binding, send, enabling any operations lane or production activation.
