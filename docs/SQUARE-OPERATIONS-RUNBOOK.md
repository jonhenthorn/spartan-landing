...
Status: **the bounded D1 monitor is remotely proven in the isolated sandbox, and the counts-only alert engine, read-only Queue/DLQ source and signed Apps Script health source are deployed inertly on schema `4`. The optimized Apps Script Version 4 contract is published with health disabled and passed an inert no-write interval. The August 18 run failed the first enabled-inspection deadline; an August 19 follow-up passed credential-inert setup and two signed-disabled probes, then stopped under the disclosure rule before Apps health was enabled. Cleanup rotated the exposed connector signing secret, removed every health credential and returned every live capability flag to false. A final exact-selector attempt stopped before saving or sending a credential.** Project 2 production activation and another Apps-health retry remain blocked.
...
- Migrations `0001` through `0004` are applied to both sandbox operations databases. At schema rollout, runtime preserved eight monitor runs and two resolved incidents while preview remained empty. After the August 18 and August 19 stopped Apps-health runs, runtime contains 18 monitor runs, three incidents and one open `APPS_HEALTH_UNAVAILABLE` warning at occurrence one; the same incident reset its confirmation sequence after a gap longer than 540 seconds. Delivery, backup and restore rows remain zero. Remote inspection found all 27 delivery columns, both reviewed indexes, all Queue and Apps alert pairs and no foreign-key failures. Exported schema-4 copies passed SQLite integrity and foreign-key checks. The sandbox intentionally omits R2 because that account feature and the backup lane are not approved; production retains the future placeholder.
- Current sandbox Worker version `d600bb6e-2a54-44c5-addd-2d3ada1ed393` is bound to runtime operations D1 `2e2fc9f6-0a81-453b-9af6-8d4104965f8e` and connector D1 `9531221e-cabe-4ed4-b7d4-f715798b8945`, has only a scheduled handler, schema `4`, no secret/Queue/email/R2 binding and every capability false. The final `03:10` UTC trigger plus settling time left all counts and prior update timestamps unchanged, including zero alert deliveries.
...
The schema-4 Apps candidate is a separate signed, read-only contract. It posts only `response_mode`, operation, health-contract version, source environment, timestamp and nonce plus HMAC-SHA256 signature to one exact `https://script.google.com/macros/s/<deployment>/exec` URL. The body is at most 2 KiB. The Worker accepts only a `302` or `303`, bounds and validates the redirect to `https://script.googleusercontent.com/macros/echo`, then performs a GET with no signed body or authorization header. One five-second deadline covers both requests. The final JSON must be at most 8 KiB, have the exact content type, insertion order, fields and types, echo the request nonce/timestamp, be within plus or minus 300 seconds, identify the exact environment/service/contract versions, prove `read_only=true` and `writes_performed=0`, and carry a valid response HMAC from the dedicated health secret.

The Apps inspection uses Google Spreadsheet metadata reads, including the reviewed ledger schema/format checks, and batches each ledger tab's format scan into one Spreadsheet read. The August 18 run did not prove reliable completion inside the shared five-second deadline: the first enabled inspection took `5016 ms` and the immediate repeat took `3878 ms`. A local undeployed exact-semantic optimization now reuses already-read secret values, skips disabled optional-lane properties and enumerates workbook tabs once, while preserving the full used-range formula and allocated-row identifier-format checks. Before another activation attempt, publish that reviewed source while disabled and prove it inert, then test the first enabled inspection and immediate repeat under a newly approved worksheet; alternatively, make a separately reviewed timeout change. A timeout is an unavailable signal, never a healthy result.

Transport, HTTP—including an unsupported redirect status—timeout, response content type, malformed JSON or a valid signed `DISABLED`/`FAILED` inspection produces `APPS_HEALTH_UNAVAILABLE` / `APPS_HEALTH_SOURCE_UNAVAILABLE`; its warning is deliverable only after two observations 240–540 seconds apart. An untrusted or malformed accepted redirect target, invalid signed-envelope field type, echo, environment, version, timestamp or signature produces immediate critical `APPS_HEALTH_INTEGRITY_FAILURE` / `APPS_HEALTH_AUTH_OR_CONTRACT_INVALID`. A valid signed `COMPLETE` inspection that differs from the five expected component states produces immediate critical `APPS_CONFIGURATION_UNHEALTHY` / `APPS_RUNTIME_CONFIGURATION_UNHEALTHY`. An unavailable or integrity-failed sample resolves none of the Apps incidents; a valid complete mismatch resolves source and integrity only; a fully expected response resolves all three.
...
Not yet remotely covered by this slice: successful end-to-end Apps acceptance, enabled Cloudflare Queue/DLQ depth, external credential/provider health, live external alert delivery, backup/restore freshness, or ledger/group comparison beyond the bounded connector codes. Queue and Apps source code and local tests are complete, and the optimized Apps contract plus schema-4 Worker are deployed inertly. The August 18 run failed the first enabled-inspection deadline. The August 19 run proved credential-inert behavior and two signed `DISABLED` calls within budget, then stopped under the disclosure rule before Apps health was enabled; a final exact-selector attempt stopped before saving or sending a credential. Forced-failure, mismatch and recovery remain untested. Another retry requires a separately reviewed owner-controlled property workflow and new explicit approval. A monitor run is never `HEALTHY` when a configured source is unavailable.
...
- [ ] Apps-health acceptance remains incomplete. The August 18 run stopped at `5016 ms`. The August 19 follow-up passed a credential-inert interval and signed `DISABLED` probes at `1791 ms` and `1009 ms`, recorded one warning with zero deliveries, then stopped when private Apps configuration appeared in automation output. Apps health was never enabled; failure/mismatch/recovery were not run. Cleanup removed all health bindings/properties, rotated the exposed connector signing secret, returned all flags false and proved a no-write cron. A final exact-selector attempt stopped before any credential save/send. Another retry requires a new explicit approval.
- [x] Implemented, locally validated, published and inertly proved an exact-semantic Apps inspection optimization that removes duplicate/disabled property reads and repeated tab enumeration without narrowing formula or allocated-row format checks. It has not passed the signed enabled-latency gate.
...
- [x] Emergency rollback returned all flags to false, removed the dedicated health URL/secret/property, rotated the exposed connector signing secret and preserved aggregate evidence. The Apps-health warning intentionally remains open at occurrence one; there are zero deliveries, backups and restores.
...
The operations plane is done only when it can detect and externally report the required connector failures, maintain verified private backups, prove isolated restoration and clean rollback, all without PII duplication or a public surface. Every alert and backup must have age, delivery/integrity and recovery evidence. Default-off must remain a tested zero-operation state. The remote proof approves only the inert schema-4 sandbox deployment and disabled Apps contract. The stopped signed runs do not approve another Apps health credential/request, Queue token/request, email binding, send, enabling any operations lane or production activation; a fourth Apps attempt requires new explicit approval.
