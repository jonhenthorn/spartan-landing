...
- [x] Local schema-5 provider-outcome code and migrations pass default-off, both Apps HTTP hops, all centralized Square requests, combined threshold, latched auth/scope, producer-liveness, retention, privacy and schema-preservation validation. This is source review only; it is not deployed or remotely accepted.
...
- [ ] Apply connector migration `0004` and operations migration `0005` under a separately approved preview-first window, prove default-off zero writes, run bounded provider-outcome acceptance, then return both monitoring and journaling/retention controls to false.
...
- [x] At the accepted schema-4 commit, the Apps health, Apps Script, operations, connector, frontend, form-backend and site validators plus both Wrangler dry-runs passed for the deployed schema-4 source. The current local schema-5 source and migrations have their own validation evidence above but are not deployed; the inert remote proof remains separate from source activation.
