...
Status: **the bounded D1 monitor is remotely proven in the isolated sandbox, and the counts-only alert engine, read-only Queue/DLQ source and signed Apps Script health source are deployed inertly on schema `4`. The matching Apps Script Version 3 contract is published with health disabled. Every live capability flag is false; there is no Queue token, Apps health secret/URL in the operations Worker, alert sender/binding, recurring backup or restore automation, and no signed Apps health request has occurred.** Project 2 production activation remains blocked.
...
- No checked or deployed operations configuration contains an email binding, sender, recipient, Queue binding/token, Apps health URL or Apps health secret. The Queue metrics account/main/DLQ IDs and Apps expected states are non-secret reviewed configuration only; both source flags are false. The sandbox Apps project now contains the reviewed Version 3 contract with `OPS_HEALTH_ENABLED=false` and environment `sandbox`, but no dedicated health shared secret. Production was not changed.
...
Not yet remotely covered by this slice: enabled Cloudflare Queue/DLQ depth, external credential/provider health, signed Apps Script probing, live external alert delivery, backup/restore freshness, or ledger/group comparison beyond the bounded connector codes. Queue and Apps source code and local tests are complete, and the Apps contract plus schema-4 Worker are deployed inertly. Configuring the existing Apps URL, installing the dedicated health secret, sending signed health requests, and creating/proving the Queue token remain separate gates. A monitor run is never `HEALTHY` when a configured source is unavailable.
...
- [ ] After separate approval, execute `docs/APPS-HEALTH-SANDBOX-ACCEPTANCE.md`: configure the existing sandbox Apps deployment URL as Worker `OPS_APPS_SCRIPT_HEALTH_URL`, install one new dedicated secret as Apps `OPS_HEALTH_SHARED_SECRET` and Worker `OPS_APPS_SCRIPT_HEALTH_SHARED_SECRET`, prove disabled/healthy/failure/mismatch/recovery behavior within the five-second deadline, and restore `OPS_HEALTH_ENABLED=false`, `OPS_APPS_SCRIPT_MONITORING_ENABLED=false` and `OPS_MONITORING_ENABLED=false`. No signed health request is approved or complete yet.
