...
Not yet remotely covered by this slice: successful end-to-end Apps acceptance, enabled Cloudflare Queue/DLQ depth, external credential/provider health, live external alert delivery, backup/restore freshness, or ledger/group comparison beyond the bounded connector codes. Queue and Apps source code and local tests are complete, and the optimized Apps contract plus schema-4 Worker are deployed inertly. The latest enabled schedule stopped after one `APPS_HEALTH_SECOND_HOP_UNAVAILABLE` row, and the later strict local probe again missed five seconds; forced-failure, mismatch and recovery remain untested. Another retry requires new explicit approval and one exact decision: retain the five-second transport ceiling and replace or abandon this synchronous source, or approve a reviewed ten-second transport ceiling with a `<8 s` acceptance SLO. A monitor run is never `HEALTHY` when a configured source is unavailable.
...
The operations plane is done only when it can detect and externally report the required connector failures, maintain verified private backups, prove isolated restoration and clean rollback, all without PII duplication or a public surface. Every alert and backup must have age, delivery/integrity and recovery evidence. Default-off must remain a tested zero-operation state. The remote proof approves only the inert schema-4 sandbox deployment and disabled Apps contract. The stopped signed runs do not approve another Apps health credential/request, Queue token/request, email binding, send, enabling any operations lane or production activation. A new Apps attempt requires explicit approval of either replacing/abandoning the synchronous five-second source or a reviewed ten-second transport ceiling with a `<8 s` acceptance SLO.
