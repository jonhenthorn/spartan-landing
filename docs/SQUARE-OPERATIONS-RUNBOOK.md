...
Status: **the bounded D1 monitor is remotely proven in the isolated sandbox, and the counts-only alert engine, read-only Queue/DLQ source and signed Apps Script health source are deployed inertly on schema `4`. The optimized Apps Script Version 4 contract is published with health disabled. Earlier August 19 diagnostics isolated a Google second-hop failure and variable end-to-end latency; those attempts hard-stopped and remain preserved below. The subsequently approved Option B implementation completed the full sandbox worksheet. Current operations version `12bd4dc9-3ed7-47e0-9c48-0c33d8a5c166` is all-off and secretless, Apps health is disabled with no dedicated property, and the final cron wrote nothing. A newer schema-6 candidate adds local default-off provider outcomes, four aggregate connector-control signals and a separately gated monthly alert-path test; migration `0005`, migration `0006`, those signals and that test are not deployed or remotely accepted. A separate backup/restore safety contract is also local and compiled shut.** Project 2 production activation remains blocked; provider monitoring, Queue access, alerts, live backups/restores and production each require separate approval and acceptance.
...
- Monitoring runs only when its flag is true and the exact five-minute cron fires. Live schema 4 requires a separate source flag for Queue metrics and another for Apps health; either source flag requires aggregate monitoring. The local schema-6 candidate adds `OPS_PROVIDER_MONITORING_ENABLED`, which also requires aggregate monitoring and reads only one heartbeat-plus-count snapshot, plus `OPS_ALERT_PATH_TEST_ENABLED`, which additionally requires the alert engine. The four new connector-control signals remain inside aggregate monitoring and add no provider access. Queue access additionally requires three exact resource IDs and a deploy-only Queues Read token. Apps access requires one exact Google deployment URL, a dedicated deploy-only health secret and reviewed expected states. The deployed but disabled alert engine can run only on that same cron, requires monitoring and two distinct role bindings plus a bounded sender. A wrong or missing cron touches none of those bindings or credentials. Backup and restore flags remain fail-closed because those lanes are not implemented in the Worker; the separate local contract is not wired to either flag.
- Migrations `0001` through `0004` are applied to both sandbox operations databases. Local migrations `0005_provider_monitoring_alerts.sql` and `0006_connector_control_alerts.sql` are unapplied. Migration `0005` adds only three fixed provider pairs; migration `0006` adds only four fixed connector-control pairs plus the monthly test pair and `TEST` delivery kind, while preserving the full prior delivery table. At schema-4 rollout, runtime preserved eight monitor runs and two resolved incidents while preview remained empty. Earlier stopped Apps-health runs brought runtime to 22 monitor runs, three incidents and one open warning. The accepted Option B worksheet brought final runtime state to 34 monitor runs, five incidents, zero active incidents and zero delivery, backup or restore rows. Five signed-disabled rows progressed one warning episode from occurrence one to five; the healthy, controlled-failure/recovery and configuration-mismatch/recovery phases then produced and resolved the expected fixed conditions. The latest persisted `12:40` and `12:45` UTC rows were source-off connector-only `ALL_CLEAR`; the final `12:50` UTC all-off cron changed no count or prior timestamp. Remote inspection found all 27 delivery columns, both reviewed indexes, all Queue and Apps alert pairs and no foreign-key failures. Exported schema-4 copies passed SQLite integrity and foreign-key checks. The sandbox intentionally omits R2 because that account feature and the live backup lane are not approved; production retains the future placeholder.
- Current sandbox Worker version `12bd4dc9-3ed7-47e0-9c48-0c33d8a5c166` is bound to runtime operations D1 `2e2fc9f6-0a81-453b-9af6-8d4104965f8e` and connector D1 `9531221e-cabe-4ed4-b7d4-f715798b8945`, has only a scheduled handler, schema `4`, no secret/Queue/email/R2 binding and every capability false. The `12:50` UTC trigger plus settling left all counts and prior update timestamps unchanged, including zero alert deliveries.
...
Use a completed private copy of the default-NO-GO [Project 2 operations activation decision record](PROJECT-2-OPERATIONS-ACTIVATION-DECISION-RECORD.md) for exactly one selected monitoring, alert, backup, restore or operational-control lane. It separates source implementation, inert provisioning/deployment, live acceptance and continuing operation; the blank template and an earlier sandbox result grant no authority.
...
The remotely accepted schema-4 evaluator uses four fixed, aggregate-only connector D1 queries. The local schema-6 candidate adds four more aggregate-only queries; none selects payloads, cursor values, customer/claim/submission/coupon/reference/order/payment/refund IDs or contact data. Together the local source evaluates:
...
- nonterminal webhook/outbox work that has reached retry attempt 3;
- a `PROCESSING` lease that remains expired after the next five-minute recovery cycle;
- `STAFF_LOOKUP_REQUIRED` at 3 in one UTC day or above 10% after at least 10 offers;
- two reviewed discount-policy rejection results within 24 hours, producing the fixed offer/pass-disable condition while leaving the manual coupon available; and
...
The local schema-6 provider candidate records only fixed `AUTH_401`, `SCOPE_403`, `RATE_429`, `SERVER_5XX` or `OTHER` class, UTC time and count after an upstream fetch was attempted. When journaling is enabled, every Square or Apps provider attempt first admits one opaque `PENDING` row immediately before its first fetch. Admission failure prevents the fetch. One atomic finalization optionally records the fixed outcome, refreshes the producer heartbeat and closes that exact row; ambiguity leaves the row `PENDING` or marks it `FAULTED`. Square `401/403` is immediately critical and remains latched for controlled credential review; it cannot disappear merely because a 15-minute row ages out. Three combined Square or Apps `429/5xx` outcomes in `(now-15m, now]` produce one warning. Apps contributes only first- or second-hop `429/5xx`; local configuration, signing, contract and network failures are not misclassified. The monitor may resolve source/rate conditions only when the same default-off connector producer has a canonical heartbeat no more than ten minutes old and no unresolved attempt proof. Missing, stale, future or unreadable heartbeat evidence, or any `PENDING`, `FAULTED`, invalid-time or future attempt row, is source-unavailable and resolves nothing. A separate default-off retention flag can prune only finalized aggregate outcome rows older than 30 days after journaling is disabled while deleting active-producer proof; it never deletes unresolved attempt proof, which requires separate controlled reconciliation. No URL, hop, HTTP status, operation, provider/customer identifier, payload, token, body or raw error is stored. The attempt row contains only its opaque UUID, fixed state and canonical UTC admission time. This is local code evidence only; migrations `0005` and `0006`, their flags and remote provider-monitoring acceptance remain unapplied or incomplete.
...
Not yet remotely covered by this slice: enabled Cloudflare Queue/DLQ depth, the local schema-6 provider and connector-control additions, the monthly alert-path test, live external alert delivery, backup/restore freshness, or ledger/group comparison beyond the bounded connector codes. The optimized Apps contract and Option B Worker completed end-to-end sandbox acceptance, including signed disabled, healthy, controlled-failure, configuration-mismatch and recovery states. Cleanup returned Apps health and all six live Worker capabilities to false, removed the temporary credential and left no active Apps incident. A monitor run is never `HEALTHY` when a configured source is unavailable or SLO-exceeded.
...
The local schema-6 planner can create at most one labeled `TEST` delivery per role per UTC month only when monitoring, alerts and the separate alert-path-test flag are all enabled. Its local validator proves duplicate-month suppression and role isolation without sending. Still deferred and unchecked: schema-6 deployment, sender/domain onboarding, both deploy-only role bindings and destinations, real owner/backup delivery acceptance, the monthly live `TEST`, a daily digest and independent alert-transport self-monitoring. Cloudflare account notifications may eventually supplement this path, but they do not replace tested business-condition alerts.

### Nightly backup lane

The checked-in local contract validates the required source/export completeness, exact storage identity, integrity and atomic closure behavior using synthetic adapters only. Its public path returns `LOCAL_CONTRACT_ONLY_LIVE_NOT_READY`. No R2 binding, scheduled writer, authoritative storage adapter, deletion-manifest store or live export exists.
...
The local contract also validates isolated-target identity, literal-only replay, source and target integrity/foreign-key checks, deletion-manifest coverage, exact-key deletion proof and failure-safe cleanup. Those synthetic checks are not live restore evidence.
...
1. Set `OPS_ALERTS_ENABLED`, `OPS_APPS_SCRIPT_MONITORING_ENABLED`, `OPS_QUEUE_MONITORING_ENABLED`, `OPS_PROVIDER_MONITORING_ENABLED`, `OPS_BACKUPS_ENABLED`, `OPS_RESTORE_TESTS_ENABLED` and `OPS_MONITORING_ENABLED` to `false` in that order; verify the next scheduled invocation performs no binding/credential access or external operation. Disable connector provider journaling before its retention-only cleanup flag; remove the producer heartbeat at deactivation and prove default-off zero writes.
...
- [x] Apps-health sandbox acceptance completed under Option B. Commit `b87fa08b4e8e1e4fcf2462bc1d82cfdbbe4fea5d` passed the inert no-write gate, five signed-disabled crons, signed healthy recovery, controlled signed failure/recovery, configuration mismatch/recovery, source-off proof and final all-off no-write gate under one shared `10000 ms` deadline and strict raw `<8000 ms` acceptance. Final D1 state is 34 runs, five incidents, zero active incidents and zero deliveries/backups/restores.
- [x] Local schema-6 code and migrations pass default-off provider outcomes, both Apps HTTP hops, all centralized Square requests, combined threshold, latched auth/scope, producer-liveness, retention, the four connector-control thresholds, monthly test role isolation/deduplication, privacy and schema-preservation validation. This is source review only; it is not deployed or remotely accepted.
- [x] The local backup/restore contract passes schema/table completeness, source/export and restored integrity, literal-only replay, exact storage and target identity, deletion-manifest coverage, exact-key deletion and closure-cleanup validation. Its public path remains compiled shut; no R2, writer, manifest store, restore executor or live evidence exists.
- [ ] Apply connector migration `0004` and operations migrations `0005` and `0006` under a separately approved preview-first window, prove default-off zero writes, run bounded provider-outcome and connector-control acceptance, then return monitoring, provider journaling/retention and alert-path-test controls to false.
- [x] Implemented, locally validated, published and remotely accepted an exact-semantic Apps inspection optimization that removes duplicate/disabled property reads and repeated tab enumeration without narrowing formula or allocated-row format checks. The revised live scheduled transport and strict `<8000 ms` repeatability gates passed.
...
- [ ] After separately approved sender/destination acceptance, the default-off monthly labeled live `TEST` is proven and returned to false; daily digest and independent transport self-monitoring remain separately deferred.
...
- [x] Exposure-first cleanup returned all flags to false, removed the dedicated health URL/secret/property and temporary Keychain/clipboard/helper material, and preserved aggregate evidence. Current version `12bd4dc9-3ed7-47e0-9c48-0c33d8a5c166` is the sole 100% all-off, secretless deployment. D1 retains 34 runs, five incidents and zero active incidents, deliveries, backups or restores; connector aggregates are unchanged.
- [x] At the accepted schema-4 commit, the Apps health, Apps Script, operations, connector, frontend, form-backend and site validators plus both Wrangler dry-runs passed for the deployed schema-4 source. The current local schema-6 source and migrations have their own validation evidence above but are not deployed; the inert remote proof remains separate from source activation.
