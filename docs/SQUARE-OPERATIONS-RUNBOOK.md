...
The Apps inspection uses Google Spreadsheet metadata reads, including the reviewed ledger schema/format checks, and batches each ledger tab's format scan into one Spreadsheet read. The August 18 run did not prove reliable completion inside the shared five-second deadline: the first enabled inspection took `5016 ms` and the immediate repeat took `3878 ms`. Before another activation attempt, narrow the reviewed metadata inspection or make a separately reviewed timeout change, then prove the first enabled inspection and immediate repeat both meet the updated gate. A timeout is an unavailable signal, never a healthy result.
