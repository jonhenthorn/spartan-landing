...
Before any remaining live sandbox case, the owner must complete a private copy of the blank default-NO-GO [baseline migration decision and closure record](PROJECT-2-BASELINE-MIGRATION-DECISION-RECORD.md) for one supervised UTC window. The bounded procedure first uploads one exact current all-off target without changing traffic, then performs a read-only source/target check, and only then may a separate final owner `GO` move sandbox traffic from the exact legacy all-off source to that target. That same final `GO` must preauthorize the fixed standalone exact-legacy recovery if the traffic assignment is interrupted or ambiguous; recovery admits only the exact source or target at 100% and refuses drift, split traffic or a third version. Neither action can run F-02, enable a control, send a request, change a secret, touch business data or affect production. The technical procedure and fixed results are in [Square sandbox fault hooks](SQUARE-SANDBOX-FAULT-HOOKS.md).
...
1. Complete, approve and close one private copy of the baseline migration record; preserve both versions and the private evidence.
