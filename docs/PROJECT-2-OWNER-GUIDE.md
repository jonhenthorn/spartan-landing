...
- The latest recorded checks show the sandbox connector on the audited legacy all-off version at 100% traffic. Its only allowed difference from the current all-off configuration is a missing explicit false fault flag.
...
- That retained-target exception has no final-deployment authority. No traffic switch, sandbox case, secret change, provider request, Queue or D1 mutation, Apps action, Brevo action or production action was authorized.
...
Before any remaining live sandbox case, the owner must complete a private copy of the blank default-NO-GO [baseline migration decision and closure record](PROJECT-2-BASELINE-MIGRATION-DECISION-RECORD.md) for a new supervised UTC window. For the retained August 21 target, the owner must first explicitly accept the narrow `RETAINED_PREPARATION_EXCEPTION_ACCEPTED` package: preserved rejection, combined command/session and version evidence identifying the one in-window upload and no second in-window upload, no retry, recorded checkpoints showing the retained exact target at 0% and exact source at 100%, scoped independent review and a fresh separate `READY_SANDBOX_LEGACY_TO_CURRENT_ALL_OFF_MIGRATION` result. That acceptance is not final approval. Only a separate final owner `GO`, after every Queue, webhook/outbox, subscription, ingress, rollback and credential gate is current, may move sandbox traffic from the exact legacy source to the exact target. That same final `GO` must preauthorize the fixed standalone exact-legacy recovery if the traffic assignment is interrupted or ambiguous; recovery admits only the exact source or target at 100% and refuses drift, split traffic or a third version. Neither action can run F-02, enable a control, send a request, change a secret, touch business data or affect production. The technical procedure and fixed results are in [Square sandbox fault hooks](SQUARE-SANDBOX-FAULT-HOOKS.md).
...
1. Explicitly accept or reject the retained preparation exception in a private baseline migration record, then separately approve and close one final-deployment window; preserve both versions and all private evidence.
