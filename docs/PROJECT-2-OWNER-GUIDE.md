...
1. Complete and sign one private activation record for the next named sandbox case. F-02 declined consent is the lowest-risk next case because its request path must stop before Turnstile, Square, Apps or provider calls and before any Queue or D1 mutation. Its new default-off coordinator separately performs only the approved aggregate read-only Queue and D1 evidence checks, waits for the exact sandbox candidate, sends exactly one canary-bound consent-`no` request, refuses retries after an ambiguous request and finishes only after the watcher confirms stable zero local change. Before final `GO`, the offline direct managed pseudo-terminal validator must pass from the exact reviewed commit. It requires Python 3.9 or newer on macOS or Linux, uses only dummy inputs and grants no live authority. The live launch must be manual at each prompt and may not use Expect, pexpect, Tcl, AppleScript, browser/UI automation, a pipe, heredoc or shell prompt matcher. The normal production request order remains unchanged.
