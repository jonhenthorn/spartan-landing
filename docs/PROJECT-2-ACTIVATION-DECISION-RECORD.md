...
For F-02 Keychain mode, after that source PASS and before namespace initialization, console access or credential creation, run `node scripts/manage-project2-f02-keychain.mjs --preflight-macos-pasteboard <fresh-namespace>`. The first acknowledgement is `START_F02_COPY_TEST_ONCE` and occurs before any pasteboard access. Type both fixed preflight acknowledgements manually; do not paste them. Their dedicated visible nonsecret reader renders only the known canonical next character after that exact character is received, supports bounded safe backspace, rejects a wrong, control or extra character before rendering it, and never writes a raw user-supplied byte. After initial clear-and-verify, the command hashes a fresh 128-bit nonce with the private namespace under a fixed domain and prints `ACTION=DRAG_SELECT_ONLY_THE_COMPLETE_NEXT_LINE_WITHOUT_LINE_BREAK`, then only the compact nonsecret `F02P1:<32hex>` result on its own line, followed by `ACTION=PRESS_COMMAND_C_THEN_TYPE_VERIFY_ACKNOWLEDGEMENT_MANUALLY_DO_NOT_PASTE`; the rendered challenge does not disclose the namespace. Drag-select only that complete line without either line break and transfer it only with native macOS Command-C or Edit > Copy, then manually type `VERIFY_F02_COPY_TEST_ONCE` without another Copy or paste action. The preflight reads only `/usr/bin/pbpaste`, exact-compares without trimming, rechecks freshness and finally clears and verifies; it opens no Keychain, takes no operation lock and contacts no provider. Record only `STATUS=COMPLETE RESULT=F02_MACOS_PASTEBOARD_PREFLIGHT_VERIFIED_AND_CLEARED`, its UTC time and a non-identifying route reference, then immediately initialize that same namespace. `F02_MACOS_PASTEBOARD_PREFLIGHT_INPUT_REJECTED`, `F02_MACOS_PASTEBOARD_PREFLIGHT_ROUTE_REJECTED`, `F02_MACOS_PASTEBOARD_PREFLIGHT_CLEAR_REJECTED`, interruption or shutdown ambiguity closes readiness before credentials and grants no retry or namespace reuse. Exit codes `20`, `21` and `22` mirror only the fixed input, route and clear rejection classes so a direct-TTY launcher can retain that public class without capturing stdout; any other nonzero or signal exit is unresolved and fail closed. A provider Copy control, browser/controller clipboard, injected clipboard write, redirected or captured stdout transcript, AppleScript or scripted retyping is not the certified route. The native route must be used separately for every staged value; `W` and `R` are distinct transfers. If Codex cannot use it without reading or exposing a private value, arrange the required per-value native Copy actions with the owner before credential creation.

Immediately after that PASS, run `node scripts/manage-project2-f02-keychain.mjs --initialize <same-namespace> <approved-window-end-utc>`. The end is public, nonsecret approval metadata and must be exact UTC seconds `YYYY-MM-DDTHH:mm:ssZ` or canonical `YYYY-MM-DDTHH:mm:ss.000Z`; seconds-only input is normalized and stored as `.000Z`. Before an operation lock, prompt, Keychain access, console check or temporary credential exists, initialization rejects offsets, local time, whitespace, lowercase `z`, nonzero or differently sized fractions, invalid dates, a stale/future namespace, an inactive window, or a duration outside one through four hours. `INITIALIZE_F02_KEYCHAIN_NAMESPACE_ONCE` uses the same canonical-only visible reader as the two preflight acknowledgements. Store, generate, cleanup, helper, operator, coordinator and retirement acknowledgements remain on their existing non-echoing readers; no credential, URL, identifier, HMAC value or other private input is eligible for visible rendering. After the initialization acknowledgement and the empty-namespace check, it rechecks the active window and five-minute namespace freshness immediately before its first Keychain write, then stores `STAGING`, the canonical end, then the exact namespace start last. Record only `STATUS=COMPLETE RESULT=F02_KEYCHAIN_NAMESPACE_INITIALIZED` before proceeding. The window end is not later staged through the pasteboard and cannot be replaced within that attempt.
...
F-02 requires two distinct temporary account-restricted credentials. `W` has only Workers Scripts Edit and is injected only into the fixed `spartan-square-connector-sandbox` Wrangler children. `R` has only Workers Scripts Read, D1 Read and Queues Read, with no corresponding write permission, and is injected only into fixed metadata, aggregate `SELECT`-only D1, topology and Queue-metrics reads. The two values must differ and may not be replaced by cached Wrangler OAuth or one combined broad credential. Their approved expiry must cover the derived closure-claim cutoff and bounded settlement/verification of a provider request started before it. Retain both only through terminal rollback, cleanup and monitored all-off closure, then revoke and prove each unusable. The Keychain storage manager stores or deletes approved values but never creates, broadens, revokes or verifies a provider credential. The separate fixed retirement verifier performs only the bounded active and post-revocation read-only checks below; it does not revoke either credential itself.
...
Use exactly one already-running retirement-verifier process for this handoff:
...
Before consuming `claim.retirement-verifier` or making a network request, it validates the retained bundle state, window, role set, credentials and required account/Queue identifiers; proves every recorded generate, helper, operator, coordinator, rollback, rollback-recovery, cleanup, cleanup-recovery and lifecycle owner dead; and requires the exact baseline rollback and cleanup completion implied by any retained provider-work claim or checkpoint. It then rechecks the owned namespace lock and half-open closure cutoff immediately before the claim and every fixed request, and rechecks the lock plus exact claim immediately after every request.

For each active HTTP `200`, the verifier reads at most 16,384 bytes as strict UTF-8 JSON. Token verification requires `success:true` and `result.status:"active"`; each Queue response requires `success:true` and nonnegative safe-integer `result.backlog_bytes`, `result.backlog_count` and `result.oldest_message_timestamp_ms`. Only after every retained role passes does it emit `READY_F02_CLOUDFLARE_TOKEN_RETIREMENT`. The named revocation owner may then delete each detected credential once in the Cloudflare console and type `VERIFY_RETIRED_F02_TOKENS_ONCE` in that same process. The verifier requires the exact one `W` plus three `R` HTTP `401` results, in fixed order, before storing the exact role-matching `checkpoint.retirement-complete`. During post-revocation proof it never reads response content, follows redirects or retries. Any failed active check or active-envelope validation, non-`401`, timeout, interruption, drift or ambiguity consumes the claim, grants no retry and blocks Keychain namespace deletion pending independent review and new authority. Namespace cleanup must occur only after the verifier completes and exits; while token items remain, cleanup requires its exact claim and role-matching completion checkpoint.

Run that verifier only in a direct managed pseudo-terminal. Handled `SIGINT`, `SIGTERM` or `SIGHUP`, prompt EOF/error/timeout, Ctrl-C/Ctrl-D or prompt I/O ambiguity aborts any in-flight authenticated request and proves every Keychain child scope zero-active before terminating the lock helper, retains the exact namespace lock marker fail-sticky and emits only `STATUS=STOPPED RESULT=F02_TOKEN_RETIREMENT_SHUTDOWN_AMBIGUOUS`. That redacted result is not retirement proof or retry authority; preserve the fence for independent exact process/provider review and separately authorized disposition.
