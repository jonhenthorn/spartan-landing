...
- A server-side Square credential stored only as an encrypted Worker secret. The local candidate accepts an access token; sandbox uses a sandbox token. Production reusable automation remains **NOT APPROVED**. If it is later activated, it must use scoped OAuth, with rotation, revocation and recovery proved through a separately scoped production owner decision; the current plan authorizes no personal access token.
...
A future production OAuth authorization would need `CUSTOMERS_READ`, `CUSTOMERS_WRITE`, `PAYMENTS_READ` and `ORDERS_READ`. It does not need payment, refund, order, catalog, card, loyalty, marketing or payout write scopes. The current [`PROJECT-2-ACTIVATION-DECISION-RECORD.md`](PROJECT-2-ACTIVATION-DECISION-RECORD.md) explicitly excludes production and grants no standing authorization. A later production-specific owner decision must record the least-privilege boundary, authority, custody, rollback owner and evidence in a completed private copy of the default-NO-GO [`PROJECT-2-PRODUCTION-ACTIVATION-DECISION-RECORD.md`](PROJECT-2-PRODUCTION-ACTIVATION-DECISION-RECORD.md) before production activation.

The separate sandbox-provider preflight is not production runtime authorization. Its frozen F-04/P-01 read-only cases require exactly `MERCHANT_PROFILE_READ` and `CUSTOMERS_READ`; its package-bound replay case requires exactly `MERCHANT_PROFILE_READ` and `PAYMENTS_READ`. Extra scopes fail closed. F-04/P-01 use `--execute-read-only --case <F-04|P-01> --ack SANDBOX_PROVIDER_READ_ONLY_PREFLIGHT_ONLY`; replay uses `--execute-read-only --case REPLAY-4XX --package "<secured-replay-package-path>" --ack SANDBOX_PROVIDER_READ_ONLY_PREFLIGHT_ONLY`. The approved temporary read-only client ID remains compiled `null`, so no live provider request is currently possible. Local validation is confined to `https://provider-fixture.invalid` and proves only request boundaries, classifications and zero mutations.

The separate OAuth lifecycle is also a local contract only. It models distinct read-only and mutation Sandbox apps, exact scopes, loopback callback validation, one-attempt admission, credential custody, full authorization revocation and access/refresh retirement proof. A validation-only bridge now passes an opaque credential handle from that lifecycle into the bounded provider helper under `https://provider-fixture.invalid`; it neither exposes a token to the fixture input nor makes a live request. The public path remains compiled shut with no reviewed dedicated Sandbox app registration, live client IDs or secret digests, OS-durable shared admission/custody adapter, trusted live clock, callback listener, provider transport or live authorization. The local bridge cannot clear the live credential gate.
...
These are proposed Phase 1 operating controls, not approved defaults. The current [`PROJECT-2-ACTIVATION-DECISION-RECORD.md`](PROJECT-2-ACTIVATION-DECISION-RECORD.md) keeps production, alert delivery and backup changes outside its one-case sandbox scope. Changing or accepting a proposal requires a later separately scoped owner decision with authority, roles, custody, rollback and evidence completed before production activation:

| Data class | Proposed Phase 1 control — not approved |
| --- | --- |
| Square access credential | Use the dedicated `Spartan First Visit Connector` application with scoped OAuth if production reusable automation is later approved. Store the runtime access credential only as a Cloudflare encrypted secret. A separately scoped production owner decision must select the external recovery-custody boundary and two separately MFA-protected roles before activation. Review access every 180 days, complete the approved rotation/revocation recovery drill at its recorded interval, and rotate immediately after suspected disclosure, owner/admin departure or unexplained authentication failures. No personal access token is authorized. |
...
The proposed recovery design uses Workers Paid so D1 Time Travel supplies a 30-day recovery window and Queue/DLQ retention can be set to 14 days. Its proposed storage lane exports D1 privately to R2 nightly with a 90-day lifecycle and exports the restricted `Identity Links` and `Journey Events` Sheet tabs monthly with the same retention. A local compiled-shut backup/restore contract now validates schema/table completeness, exact private-object and target identity, source and restored integrity/foreign keys, deletion-manifest coverage, exact-key deletion proof and atomic cleanup with synthetic adapters. It is not a writer, schedule or live restore. R2 enablement, bucket/account custody, lifecycle application and recurring export authority are **not approved** and remain unimplemented. The current one-case sandbox decision record requires these lanes to remain unchanged; a completed private copy of the default-NO-GO [`PROJECT-2-OPERATIONS-ACTIVATION-DECISION-RECORD.md`](PROJECT-2-OPERATIONS-ACTIVATION-DECISION-RECORD.md) must select and approve exactly one operations lane before implementation or live acceptance. If later approved, every migration must record a Time Travel bookmark and create an explicit SQL export; each quarter, restore D1 and the Sheet exports into isolated nonproduction copies, reconcile row counts and unique keys, record the result and remove the restore-test copies within seven days.

The proposed deletion manifest remains outside D1 and outside any R2/Sheet backup sets in a restricted, business-owned record accessible only to two MFA-protected roles. Its storage system and role custody are **not approved** and must be selected in that separate operations owner decision. If later approved, it stores only internal `claim_id`, deletion effective date, affected record class, policy version and a bounded reason code—no name, raw contact data or Square ID—and retains each entry for 120 days, covering the proposed 90-day backup lifecycle plus a 30-day safety margin. Deleted customer-linked data may remain only in encrypted backups until their normal approved expiry. Any restore must apply every still-retained manifest entry before the connector is re-enabled so an old backup cannot silently resurrect an unlinked record.
...
- The current one-case sandbox decision record is `NOT APPROVED`, explicitly excludes production and does not authorize implementing the proposed Phase 1 credential, retention, backup, deletion or suppression controls. Each later operations lane requires its own completed private [operations decision record](PROJECT-2-OPERATIONS-ACTIVATION-DECISION-RECORD.md), and the production owner canary requires its own completed private [production decision record](PROJECT-2-PRODUCTION-ACTIVATION-DECISION-RECORD.md), with exact authority, roles, custody, rollback and evidence before implementation or activation.
...
- A sandbox-only Apps shared secret that appeared in diagnostic output was rotated immediately in Apps Script and the Worker. Later Apps-health work disclosed two replacements in automation output; each was revoked and replaced in both disabled sandbox stores. The temporary sandbox transaction authorization was revoked. No exposed credential value or private Apps locator is retained in Git or in this record.
- The completed core-path exercise used sandbox Worker version `ef14512d-35c4-4570-b6bd-e9768585c8ae` in deployment `8ec3705a-9428-46d3-9aff-dc727ffff559`. For the later reconciliation-only proof, only consumer and reconciliation were temporarily set to `true`; offer, pass, webhook and owner-harness remained `false`, canary-only retained an empty allowlist, the Square subscription remained disabled and Apps `SQUARE_JOURNEY_ENABLED=false`. Reconciliation was then turned off first, terminal/dead state was rechecked, and consumer was turned off. Worker version `6081f8ec-3626-4731-aa3d-c1abf065e2d9` was the historical all-off result of that proof. Worker version `0ff5a2ab-2f2c-4872-a624-29d976ab54de`, created by the August 19 remediation rotation and bound to runtime D1 `9531221e-cabe-4ed4-b7d4-f715798b8945`, was the next historical all-off baseline. The closed August 21 baseline migration then made one separately reviewed current all-off target the sole sandbox baseline. Its exact current version and binding references remain in the private evidence record rather than this shared document. The documented current boundary is all five automation flags and owner-harness `false`, canary-only `true` with an empty allowlist, and public config `enabled:false`; any live window must independently reverify that boundary and the privately recorded exact rollback target.
...
- The canonical `npm run validate` suite passes, including connector outage, partial group/ledger recovery and forged/altered/unrecognized webhook cases, plus the Apps Script, website frontend, form backend and full-site validators. These local checks support the implementation but are not substitutes for the remaining live provider tests. The unexecuted live matrix and its current blockers are recorded in `SQUARE-SANDBOX-NEGATIVE-RECOVERY-ACCEPTANCE.md`.
- The local provider helper includes fixed read-only results `F04_NEW_CUSTOMER_SLOT_CLEAR`, `P01_NEW_CUSTOMER_SLOT_CLEAR` and `REPLAY_PERMANENT_SQUARE_REJECTION_READY`. They have not been produced live. F-04/P-01 results would be point-in-time search evidence only; replay would accept only an authorized target `400`–`499` other than `401`, `403` or `429` for the exact revalidated package. `401` remains `AUTH_REJECTED`, `403` remains `SCOPE_REJECTED`, and neither may masquerade as target evidence. None of these results proves object absence, Apps readiness, a provider mutation, live deployment or case acceptance.

Still required before sandbox signoff and any production owner canary:

- Register and independently accept the dedicated temporary OAuth apps and live issuance/custody/revocation adapters, replace the validation-only `.invalid` bridge with a reviewed OS-durable live integration and run the read-only preflight inside a separately approved evidence window. The current compiled-null public gate means F-04, P-01 and recognized replay remain blocked despite the composed local contract.
...
- The one-time D1 export/restore integrity drill and local safety contract are complete. R2/storage/manifests are still absent, so recurring private backups, backup-age monitoring, live deletion-manifest application, Sheet export/restore, external-alert sender/destination configuration and end-to-end delivery proof, and full production owner rollback still must be implemented and demonstrated. Any future production OAuth authorization must also demonstrate refresh, rotation and full-revocation behavior before activation.
...
The isolated `spartan-square-ops-sandbox` Worker proves aggregate D1 monitoring for overdue webhook/outbox work, dead outbox rows, bounded rejection classes, reconciliation overflow/freshness, source failure and recovery. It persists only fixed codes, aggregate counts and times. It has no public route and remains deployed with every capability false. Alert migration `0002` and the two-role counts-only planner/drainer are applied and deployed inertly in sandbox, but no email sender, role binding, recipient, flag change or send exists. Migration `0003` and a Queue/DLQ metrics source are also deployed inertly with no token and the source flag false. Migration `0004`, the schema-4 signed Apps health source and the matching disabled Apps Script Version 4 optimized contract are deployed. Earlier August 18 and August 19 attempts remain preserved in the acceptance worksheet. In an earlier enabled worksheet run, Worker `2c5c7fa7-be5b-44ef-9fac-1b00fdd51920` produced one row from `06:30:16.250Z` through `06:30:19.216Z` (`2966 ms`) with `APPS_HEALTH_SECOND_HOP_UNAVAILABLE`; Apps Version 4 `doPost` completed in `2.069 s`, but the required signed-disabled scheduled result was not reached and no second row ran. Detailed split commit `76510a0` was deployed inertly as `d90fcd45-ac10-4800-b14b-c4bd882df554` and passed the `06:55` no-write interval. A later local-only diagnostic returned signed `DISABLED` in `5422 ms` and `1585 ms`, while the normal strict probe stopped at `5011 ms` before any Worker health secret or flag change. Cleanup removed the Apps property and temporary Keychain material. At the close of that stopped attempt, operations version `d90fcd45-ac10-4800-b14b-c4bd882df554` was the sole 100% deployment, had no Apps URL/shared secret and all six flags false; D1 preserved 22 monitor runs, three incidents and one open warning occurrence one, with zero deliveries/backups/restores. Connector aggregates and production remained unchanged.

The subsequently approved Option B implementation in commit `b87fa08b4e8e1e4fcf2462bc1d82cfdbbe4fea5d` uses one shared `10000 ms` transport deadline, no retry and strict raw `<8000 ms` acceptance; exact `8000`–`9999 ms` signed responses become fixed unavailable evidence, while integrity failure remains critical. Inert version `4225524f-5c2d-44ac-b37c-bbeb2721a09d` passed its no-write interval. Enabled version `1309eeed-cbb7-4daa-8c41-8eb7f872a616` then passed five signed-disabled scheduled observations, signed healthy recovery, controlled signed failure and recovery, and—through mismatch version `f52ec4f4-d4c5-4753-a7e2-169928a35998`—configuration-mismatch and recovery. Every qualifying direct and scheduled observation completed below eight seconds and no delivery occurred. Source-off version `f3df1f27-d217-48a4-9926-0aabb15b0561` produced connector-only `ALL_CLEAR` rows at `12:40` and `12:45` UTC with no Apps request or incident change. Cleanup removed both Worker health secrets, the Apps property and temporary credential material. Final operations version `12bd4dc9-3ed7-47e0-9c48-0c33d8a5c166` is scheduled-only, schema 4, secretless and all six flags false; the `12:50` UTC cron wrote nothing. D1 now preserves 34 monitor runs, five incidents, zero active incidents and zero deliveries/backups/restores. Connector `0ff5a2ab-2f2c-4872-a624-29d976ab54de`, its aggregates and production remained unchanged. The Apps-health sandbox lane is complete. A newer local schema-6 source retains fixed-class Square `401/403` plus combined Square/Apps `429/5xx` monitoring and adds aggregate retry-attempt, stale-lease-survivor, staff-lookup and discount-policy signals. It also contains a separately gated, at-most-once-per-month alert-path `TEST` for each owner role. Operations migrations `0005` and `0006`, connector migration `0004`, those signals and that test are not migrated, deployed, sent or remotely accepted. Queue acceptance, credential-age and ledger/group comparison sources remain separate gates. Ordinary customer inactivity is not a technical alert.
...
2. Keep verified webhook ingest/consumer processing enabled only long enough to finish already accepted events; it writes durable state and may call the reviewed Square/Apps actions, so it is not a read-only phase. Disable it immediately for an active security incident. Project 2 has no reward automation.
3. Drain/reconcile ordinary accepted Queue work through the bounded connector path. Handle a dead-letter item only through the sandbox exact-target inspect/redrive approval in `SQUARE-DLQ-REDRIVE.md`; never retry an ambiguous push blindly or imply a general replay command. Append corrections or mark links inactive. Do not delete Square customers, payments, Sheet rows or ledger events as rollback.
...
The completed core path, one-time D1 restore, local OAuth/provider/backup safety contracts, two-cycle reconciliation, isolated flag-order rollback and aggregate D1 monitor do not satisfy the overall definition of done. Physical checkout-device scanning, the remaining live recovery matrix, provider integration, broader monitoring sources, external alert delivery, recurring backup and backup-age controls, retention/deletion operations, Sheet restore, full production owner rollback and the production owner canary remain open.
