...
- A sandbox-only Apps shared secret that appeared in diagnostic output was rotated immediately in Apps Script and the Worker. A later Apps-health preflight exposed its replacement in automation output, so it was revoked and rotated again before any health credential or flag change. The temporary sandbox transaction authorization was revoked. No exposed credential value is retained in Git or in this record.
- The completed core-path exercise used sandbox Worker version `ef14512d-35c4-4570-b6bd-e9768585c8ae` in deployment `8ec3705a-9428-46d3-9aff-dc727ffff559`. For the later reconciliation-only proof, only consumer and reconciliation were temporarily set to `true`; offer, pass, webhook and owner-harness remained `false`, canary-only retained an empty allowlist, the Square webhook subscription stayed disabled and Apps `SQUARE_JOURNEY_ENABLED=false`. Reconciliation was turned off first after the second cycle, zero nonterminal/dead state was reconfirmed, and consumer was turned off. Worker version `6081f8ec-3626-4731-aa3d-c1abf065e2d9` was the historical all-off result of that proof. Current connector version `cf69ea69-4d5e-4aae-8e59-b85341b8eb3a`, created by the later remediation rotation, is bound to runtime D1 `9531221e-cabe-4ed4-b7d4-f715798b8945`; all five automation flags and owner-harness are `false`, canary-only is `true` with an empty allowlist, and public config reports `enabled:false`.
...
The isolated `spartan-square-ops-sandbox` Worker proves aggregate D1 monitoring for overdue webhook/outbox work, dead outbox rows, bounded rejection classes, reconciliation overflow/freshness, source failure and recovery. It persists only fixed codes, aggregate counts and times. It has no public route and remains deployed with every capability false. Alert migration `0002` and the two-role counts-only planner/drainer are applied and deployed inertly in sandbox, but no email sender, role binding, recipient, flag change or send exists. Migration `0003` and a Queue/DLQ metrics source are also deployed inertly with no token and the source flag false. Migration `0004`, the schema-4 signed Apps health source and the matching disabled Apps Script Version 3 contract are deployed. The approved August 18 health run passed signed-disabled transport checks but failed the first enabled-inspection deadline at `5016 ms`; its immediate repeat was healthy in `3878 ms`, which did not override the stop rule. Forced-failure, mismatch and recovery phases were not run. Final operations version `65e97390-997e-46e8-9afa-f8721c644ef0` has no Apps URL/shared secret and all six flags false; D1 preserves 17 monitor runs, three incidents and one open warning occurrence nine, with zero deliveries/backups/restores and a verified all-off no-write cron. The contract persists no URL, secret, body, nonce, signature, raw error, Sheet identifier or customer data. Another Apps retry requires offline remediation and new approval. Queue acceptance and the missing credential-age/provider-rate and ledger/group comparison sources remain separate gates. Ordinary customer inactivity is not a technical alert.
