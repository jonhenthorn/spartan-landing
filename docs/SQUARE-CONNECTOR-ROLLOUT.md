...
The isolated `spartan-square-ops-sandbox` Worker now proves aggregate D1 monitoring for overdue webhook/outbox work, dead outbox rows, bounded rejection classes, reconciliation overflow/freshness, source failure and recovery. It persists only fixed codes, aggregate counts and times. It has no public route and is deployed with every capability false. Alert migration `0002` and the two-role counts-only planner/drainer are applied and deployed inertly in sandbox, but no email sender, role binding, recipient, flag change or send exists. Candidate migration `0003` and a Queue/DLQ metrics source are locally validated and default off; they use two fixed Cloudflare Queue-metrics `GET` requests with a future Queues Read-only secret and no producer/consumer binding or message-operation path. They are not yet remotely migrated, deployed, credentialed or requested. Before activation, complete that bounded Queue rollout and add/prove the still-missing owner-visible sources for invalid-signature or request-freshness failures, credential age/failure, provider error-rate spikes, duplicate/ambiguous profiles, Apps health and ledger/group drift. Ordinary customer inactivity is not a technical alert.
