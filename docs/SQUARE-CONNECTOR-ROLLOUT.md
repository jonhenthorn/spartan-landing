...
- The connector validation passes 28 local checks, including explicit outage, partial group/ledger recovery and forged/altered/unrecognized webhook cases. The Apps Script, website frontend, form backend and full-site validations pass. These local checks support the implementation but are not substitutes for the remaining live provider tests. The unexecuted live matrix and its current blockers are recorded in `SQUARE-SANDBOX-NEGATIVE-RECOVERY-ACCEPTANCE.md`.
