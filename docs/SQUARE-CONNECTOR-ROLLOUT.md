...
The isolated `spartan-square-ops-sandbox` Worker proves aggregate D1 monitoring for overdue webhook/outbox work, dead outbox rows, bounded rejection classes, reconciliation overflow/freshness, source failure and recovery. It persists only fixed codes, aggregate counts and times. It has no public route and remains deployed with every capability false. Alert migration `0002` and the two-role counts-only planner/drainer are applied and deployed inertly in sandbox, but no email sender, role binding, recipient, flag change or send exists. Migration `0003` and a Queue/DLQ metrics source are also deployed inertly with no token and the source flag false. Migration `0004`, the schema-4 signed Apps health source and the matching disabled Apps Script Version 4 optimized contract are deployed. The August 18 health run failed the first enabled-inspection deadline at `5016 ms`. The August 19 follow-up passed credential-inert setup and signed `DISABLED` calls at `1791 ms` and `1009 ms`, then stopped under the disclosure rule after one scheduled unavailable warning; Apps health was never enabled. Cleanup removed every health binding/property, rotated the exposed connector signing secret and proved a final all-off interval. Final operations version `d600bb6e-2a54-44c5-addd-2d3ada1ed393` has no Apps URL/shared secret and all six flags false; D1 preserves 18 monitor runs, three incidents and one open warning occurrence one, with zero deliveries/backups/restores. A final exact-selector attempt stopped before any credential was saved or sent. The contract persists no URL, secret, body, nonce, signature, raw error, Sheet identifier or customer data. Another Apps attempt requires a separately reviewed owner-controlled property workflow and new explicit approval. Queue acceptance and the missing credential-age/provider-rate and ledger/group comparison sources remain separate gates. Ordinary customer inactivity is not a technical alert.
