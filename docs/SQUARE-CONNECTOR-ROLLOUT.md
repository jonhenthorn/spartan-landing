...
The separate OAuth lifecycle is also a local contract only. It models distinct read-only and mutation Sandbox apps, exact scopes, loopback callback validation, one-attempt admission, credential custody, full authorization revocation and access/refresh retirement proof. Its public path remains compiled shut with no registered app IDs or secret digests, trusted clock, shared durable backing, callback listener, Square transport or case adapter. It is not yet connected to the provider helper and cannot clear the live credential gate.
...
- The current one-case sandbox decision record is `NOT APPROVED`, explicitly excludes production and does not authorize implementing the proposed Phase 1 credential, retention, backup, deletion or suppression controls. Each later activation lane requires its own completed authority, role, custody, rollback and evidence decision before implementation.
...
- The completed core-path exercise used sandbox Worker version `ef14512d-35c4-4570-b6bd-e9768585c8ae` in deployment `8ec3705a-9428-46d3-9aff-dc727ffff559`. For the later reconciliation-only proof, only consumer and reconciliation were temporarily set to `true`; offer, pass, webhook and owner-harness remained `false`, canary-only retained an empty allowlist, the Square webhook subscription stayed disabled and Apps `SQUARE_JOURNEY_ENABLED=false`. Reconciliation was turned off first after the second cycle, zero nonterminal/dead state was reconfirmed, and consumer was turned off. Worker version `6081f8ec-3626-4731-aa3d-c1abf065e2d9` was the historical all-off result of that proof. Current connector version `0ff5a2ab-2f2c-4872-a624-29d976ab54de`, created by the August 19 remediation rotation, is bound to runtime D1 `9531221e-cabe-4ed4-b7d4-f715798b8945`; all five automation flags and owner-harness are `false`, canary-only is `true` with an empty allowlist, and public config reports `enabled:false`.
...
- Provision and independently accept the dedicated temporary OAuth issuance/custody/revocation adapters, then connect the local lifecycle to the bounded provider helper and run the read-only preflight inside a separately approved evidence window. The current compiled-null gate means F-04, P-01 and recognized replay remain blocked despite the local contract.
