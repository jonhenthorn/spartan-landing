...
The isolated `spartan-square-ops-sandbox` Worker now proves aggregate D1 monitoring for overdue webhook/outbox work, dead outbox rows, bounded rejection classes, reconciliation overflow/freshness, source failure and recovery. It persists only fixed codes, aggregate counts and times. It has no public route and is deployed with every capability false. Before activation, add and prove the still-missing owner-visible sources for invalid-signature or request-freshness failures, credential age/failure, Queue or dead-letter depth, provider error-rate spikes, duplicate/ambiguous profiles, Apps health and ledger/group drift. External owner and backup-owner delivery is not implemented. Ordinary customer inactivity is not a technical alert.
