...
- Cleanup removed the Apps health property and temporary Keychain URL/secret items. At the close of that diagnostic, operations version `d90fcd45-ac10-4800-b14b-c4bd882df554` was the sole 100% deployment, all six flags were false and its secret list was empty. Operations D1 then contained 22 monitor runs, three incidents, one active `APPS_HEALTH_UNAVAILABLE` warning at occurrence one, and zero deliveries, backups or restores; the latest row was the `06:30` sample. Connector aggregates and all production/business state remained unchanged.
- Acceptance remained incomplete at that evidence point. The owner subsequently approved one sandbox-only Option B qualification: a single shared ten-second transport ceiling with strict `<8 s` acceptance, no retry and immediate rollback on any miss. The accepted run below did not approve production.

#### August 19 Option B Apps-health acceptance and final cleanup

- Commit `b87fa08b4e8e1e4fcf2462bc1d82cfdbbe4fea5d` implemented one shared `10000 ms` transport deadline, no retry and strict raw `<8000 ms` acceptance. Inert operations version `4225524f-5c2d-44ac-b37c-bbeb2721a09d` had all six capabilities false and passed a no-write scheduled interval before any signed request was enabled.
- Credential setup produced test-only versions `1a55850c-b1e6-4bd5-a0b1-bca9e6f05053` and `d5380123-4c3d-42ed-9efd-e0c322b7e42d` while all capabilities remained false. Normal enabled-test version `1309eeed-cbb7-4daa-8c41-8eb7f872a616` enabled only aggregate and Apps monitoring. Direct signed `DISABLED` results completed in `2090 ms` and `933 ms`. Five scheduled disabled rows at `11:50`, `11:55`, `12:00`, `12:05` and `12:10` UTC each completed below eight seconds and advanced one warning episode from occurrence one through five without a delivery.
- Direct healthy inspections completed in `3107 ms` and `2432 ms`; the `12:15` UTC cron recorded `ALL_CLEAR`. A controlled direct signed `FAILED` result completed in `1601 ms`, the `12:20` UTC cron recorded `APPS_HEALTH_SIGNED_FAILED`, a restored healthy probe completed in `5667 ms`, and the `12:25` UTC cron returned `ALL_CLEAR`.
- Mismatch-test version `f52ec4f4-d4c5-4753-a7e2-169928a35998` returned the expected signed configuration mismatch in `3966 ms`; the `12:30` UTC cron recorded `MONITOR_CRITICAL`. Restoring normal version `1309eeed-cbb7-4daa-8c41-8eb7f872a616` produced a direct healthy result in `4617 ms`, and the `12:35` UTC cron recorded `ALL_CLEAR`. At that point D1 contained 32 monitor runs, five incidents, zero active incidents and zero deliveries, backups or restores.
- Exposure-first cleanup deployed source-off version `f3df1f27-d217-48a4-9926-0aabb15b0561`. The `12:40` and extra `12:45` UTC crons were connector-only `ALL_CLEAR` and made no Apps request or Apps-incident change. URL-delete version `cc8350a0-efc7-44b4-93fc-9aaa90dca847` and shared-secret-delete version `2e636c1f-4356-40a6-9e7b-7b5198ae999c` removed both Worker health secrets. Historical test-only versions containing encrypted health bindings must not be redeployed.
- Apps Version 4 is restored to `OPS_HEALTH_ENABLED=false` and environment `sandbox`; its dedicated health property is absent. Temporary Keychain items, clipboard value, credential helper and temporary directory were removed. Final operations version `12bd4dc9-3ed7-47e0-9c48-0c33d8a5c166` is the sole 100% deployment, scheduled-only, schema 4, bound to the exact runtime D1 databases, secretless and all six capabilities false. The `12:50` UTC cron changed no D1 count or prior timestamp.
- Final operations D1 contains 34 monitor runs, five incidents, zero active incidents and zero deliveries, backups or restores. Connector version `0ff5a2ab-2f2c-4872-a624-29d976ab54de` and all connector aggregates remained unchanged. Production, website, customer, order, coupon, Square business data, Brevo and form behavior were unchanged. This completes only the Apps-health sandbox lane; Queue access, alerts, backups/restores and production activation remain unapproved.
