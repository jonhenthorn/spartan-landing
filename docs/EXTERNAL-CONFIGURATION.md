...
- Square would not accept `Square` in an application name, so the isolated developer application was created as `Spartan First Visit Connector`. A dedicated `Spartan First Visit Sandbox` test seller was created with automatic all-application authorization cleared. Runtime authorization is limited to `CUSTOMERS_READ`, `CUSTOMERS_WRITE`, `ORDERS_READ`, `PAYMENTS_READ` and Square's mandatory `MERCHANT_PROFILE_READ`; temporary catalog/transaction fixture authorization is broader only for the supervised sandbox test window.
- Two sandbox authorizations were revoked immediately after their credentials became visible during setup. Neither exposed authorization remains valid, and no production credential was opened or changed. The sandbox Worker therefore requires a fresh least-privilege runtime token before any connector flag can be enabled.
...
- Cloudflare now has isolated runtime and preview D1 databases, a main Queue, a one-day-retention dead-letter Queue, a host-scoped managed Turnstile widget and the workers.dev-only `spartan-square-connector-sandbox` Worker. Migrations `0001`, `0002` and reviewed retry-scheduler migration `0003` are applied to both databases. Inert Worker version `4df72c7b-bd37-46c9-ae78-9692d4fbff4c` is deployed with the durable retry recovery enabled in code but every customer-facing and processing feature flag still off. The Worker has two independently generated hashing/session secrets and the Turnstile secret; its stored Square token is revoked and must be replaced before activation. Secret values are not stored in the repository.
...
- Production still has no automatic website-to-Square customer, group, scan-pass, redemption or later-purchase synchronization. The sandbox now has one verified fixture item (Tea $10, Shake $12, ineligible Add-On $2), one fixed 50% discount, separate Eligible/Redeemed groups and an allowlist containing only the Tea and Shake variation IDs. Apps Script test-ledger credentials, a fresh runtime Square token and a webhook signature remain absent, so activation remains blocked until the documented sandbox gates are completed.
- Under the durable Spartan business Google account, an owner-only sandbox folder, empty sandbox ledger Sheet and bound sandbox Apps Script project were created. The header bootstrap has not run, no web app or Script Properties were deployed, and no test claim or journey row was written because Google presented the private script's unverified-app authorization warning for owner review. Two accidental personal-account test files were moved to Drive trash before any execution or data write; production data and the business-owned sandbox objects were unaffected.
