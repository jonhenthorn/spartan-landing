...
- A fresh read-only account check found no Square Developer application and no Square Sandbox test account. No production `Website 50% Eligible` group was visible; the only matching manual group was the labeled one-customer test group.
- Cloudflare had no D1 database, Queue, dead-letter Queue, Turnstile widget or deployed `spartan-square-connector`. The existing form Worker was left untouched. A separate workers.dev-only sandbox configuration now exists locally with placeholder resources, all feature flags false and no secrets, but no external sandbox resource was created or deployed.
- Square Text Message Marketing showed `Not subscribed`, failed business verification and an active `Collect text subscribers on point of sale` prompt offering 50% after checkout. This remains a competing/dangling offer path. It must be turned off or deliberately unified before the connector canary; no setting was changed during this read-only check.
