...
- During the settings step, the pre-rotation sandbox Square-connector shared secret was rendered into automation output and must be treated as compromised. A replacement was installed in both sandbox stores while the connector remained disabled. Local clipboard and temporary variables were cleared where possible, but historical output containing the revoked value remains sensitive. Cross-system use of the replacement was not exercised during that rollout. Connector version `0d017304-0002-421b-b94a-fb3251c6c9e7` was the historical result; a later preflight exposure required a second rotation, and current connector version `cf69ea69-4d5e-4aae-8e59-b85341b8eb3a` remains fully disabled. No current secret is recorded in Git.
...
- A private Apps-property preflight rendered the then-current sandbox connector shared secret into automation output. It was treated as compromised and replaced in both disabled sandbox stores before health setup continued. Connector version `cf69ea69-4d5e-4aae-8e59-b85341b8eb3a` is at 100% with the runtime connector D1, all five automation flags and the owner harness false, canary-only true with an empty allowlist, and public configuration disabled. The deployment screen also rendered the private Apps URL; it is not an authentication credential and the HMAC health secret did not exist at that time, but the historical output remains sensitive. No current secret or private URL is recorded in Git.
...
- Emergency cleanup restored Apps health false/environment sandbox, removed `OPS_HEALTH_SHARED_SECRET`, cleared the temporary Keychain/clipboard material, and removed both Worker health secrets. Final operations version `65e97390-997e-46e8-9afa-f8721c644ef0` is at 100%, scheduled-only, schema `4`, bound only to the exact runtime D1 databases, with no secret or unexpected binding and every capability false. The next all-off cron changed no count or prior maximum timestamp.
- Final operations D1 has 17 monitor runs, three incidents and one open `APPS_HEALTH_UNAVAILABLE` warning with occurrence count nine; alert deliveries, backups and restores remain zero. The open warning is retained as failed-run audit evidence. No production, customer, order, coupon, Square, Brevo, website or form state changed. Another Apps-health retry is blocked until the metadata path is optimized or a new deadline is separately reviewed, tested and approved.
