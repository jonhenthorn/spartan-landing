...
- During the settings step, the pre-rotation sandbox Square-connector shared secret was rendered into automation output and must be treated as compromised. A replacement was installed in both sandbox stores while the connector remained disabled. Local clipboard and temporary variables were cleared where possible, but historical output containing the revoked value remains sensitive. Cross-system use of the replacement has not been exercised. The connector's resulting version `0d017304-0002-421b-b94a-fb3251c6c9e7` is at 100%, uses the runtime connector D1, retains all five automation flags false, canary-only true with an empty allowlist, and still reports public configuration disabled. No current secret is recorded in Git.
...
- Scheduled-only operations Worker version `804dae4f-44d8-45de-a6e1-6ca3182d682e` is deployed at 100% with schema `4`, the exact runtime operations and connector D1 bindings, every monitoring/Queue/Apps/alert/backup/restore flag false, and no secret, Apps URL, Queue, email, R2 or public-route binding. The 9:15 a.m. Central trigger plus settling minute left all five table counts and prior update timestamps unchanged.
