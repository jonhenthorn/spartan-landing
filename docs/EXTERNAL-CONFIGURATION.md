...
- Cloudflare has isolated runtime and preview D1 databases, a main Queue, a one-day-retention dead-letter Queue, a host-scoped managed Turnstile widget and the workers.dev-only `spartan-square-connector-sandbox` Worker. Migrations `0001`, `0002` and reviewed retry-scheduler migration `0003` are applied to both databases. Active Worker version `ef14512d-35c4-4570-b6bd-e9768585c8ae` is in deployment `8ec3705a-9428-46d3-9aff-dc727ffff559`. Its hashing, session, Turnstile, webhook-signature and Apps transport secrets exist only as encrypted provider-side secrets.
- The final sandbox deployment is live only at its dedicated workers.dev hostname and has no `spartandrink.com` route. `SQUARE_OFFER_ENABLED`, `SQUARE_WEBHOOK_ENABLED`, `SQUARE_PASS_ENABLED`, `SQUARE_CONSUMER_ENABLED`, `SQUARE_RECONCILIATION_ENABLED` and the sandbox owner-harness flag are all `false`; canary-only mode remains `true` with an empty allowlist. The exact four-event Square webhook subscription is disabled, the public config reports `enabled:false`, and the production site origin remains rejected.
...
- Production still has no automatic website-to-Square customer, group, barcode, redemption or later-purchase synchronization. The production website, form Worker, Apps deployment, Sheet data, Brevo flow and Square account were unchanged by the isolated sandbox exercise. The proven core path does not satisfy the remaining recovery, monitoring, backup/restore or production owner-canary gates in `docs/SQUARE-CONNECTOR-ROLLOUT.md`.
