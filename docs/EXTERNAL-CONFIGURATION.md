...
- Scheduled-only Worker version `a49059b4-6226-4cc9-be6e-ba65d94ab509` is deployed at 100% with schema `2`, only the runtime operations and real sandbox connector D1 bindings, and no email, Queue, R2, secret or public-route binding. All four operations capability flags and reconciliation expectation remain `false`. The next five-minute trigger left all row counts and prior update timestamps unchanged.
