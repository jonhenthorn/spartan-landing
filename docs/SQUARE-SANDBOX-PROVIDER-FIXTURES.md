...
Status: **local preparation, bounded read-only preflights and a composed validation-only OAuth/provider-fixture bridge are implemented; live provider execution remains BLOCKED in code.** No provider fixture or read-only preflight in this guide has been created, run or accepted live. `scripts/prepare-square-sandbox-provider-fixtures.mjs` is default-inert. Its five mutating fixture cases and three read-only preflight cases pass only through validator-injected transport at `https://provider-fixture.invalid`. The helper's mutation and read-only OAuth client IDs remain deliberately `null`, so every otherwise valid public `--execute` or `--execute-read-only` command returns `CREDENTIAL_GATE_BLOCKED` before prompting, creating or reading a private package, or making a request. `manage-square-sandbox-oauth.mjs` and `square-sandbox-oauth-fixture-bridge.mjs` locally validate exact case scopes, callback state, short-lived issuance, opaque-handle fixture access, full revocation and access/refresh retirement against `.invalid` adapters. Their public paths remain compiled shut: no reviewed dedicated Sandbox app registration or live configuration exists, live IDs and secret digests are absent, and there is no OS-durable shared admission/custody adapter, trusted live clock, callback listener, provider transport or live authorization. No helper deploys a Worker, enables a flag or subscription, sends a webhook, touches D1/Queue/Apps, mutates a live provider object or reaches production. A future unblock still requires the dedicated-app controls below, reviewed live integration, current live evidence and separate owner approval; this guide cannot clear that gate.
...
The credential gate is **BLOCKED for both read-only preflight and fixture mutation**. The repository contains a validation-only bridge between the local OAuth lifecycle and fixture helper, but the public execution paths still obtain, pass, refresh or revoke nothing. `APPROVED_TEMPORARY_OAUTH_CLIENT_ID` and the intentionally separate `APPROVED_READ_ONLY_OAUTH_CLIENT_ID` both remain compiled as `null`. No reviewed dedicated Sandbox app registration, live client ID/secret digest, OS-durable shared admission/custody adapter, callback listener or provider transport is configured. A Square Sandbox personal access token is broad and is forbidden. The standing connector application, access token and authorization are also forbidden: the preflight must remain isolated from runtime custody, and the fixture window requires transaction writes that must not risk the runtime authorization.
...
3. replace the validation-only `.invalid` bridge with a separately reviewed owner-side OAuth controller, OS-durable shared admission/custody backing and live provider transport, then obtain a new token with `short_lived: true`, the exact case-specific `scopes` below and no other scope;
...
`revoke_only_access_token: true` is insufficient for this model because it leaves the OAuth authorization, and therefore its refresh credential, active. Full revocation is safe here only because the future client must be dedicated and isolated from the standing connector. Until the complete proposal above is integrated, independently accepted and approved, live provider execution remains blocked. The composed local validators prove only synthetic issuance, opaque-handle custody, revocation and post-revocation behavior against `.invalid` adapters; the fixture helper's public path still performs none of those actions and there is no live adapter or live evidence.
...
The OAuth validator must identify itself as `local contract only`; empty OAuth-manager invocation must report `STATUS=INERT`, `RESULT=NO_REQUEST` and `CONTRACT=LOCAL_CONTRACT_ONLY_LIVE_NOT_READY`. An exact execute-shaped OAuth invocation must still return `CREDENTIAL_GATE_BLOCKED` with zero requests. The focused bridge validator composes that closed lifecycle with the fixture helper only through opaque handles and `.invalid` transport. These checks do not register an app, start a live callback listener, authorize a merchant, issue or revoke a live token, configure provider transport or clear either compiled public gate.
